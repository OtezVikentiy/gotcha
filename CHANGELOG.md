...
- Maintenance windows now silence notifications from every source in the project — uptime, hosts, metrics, regressions, profiles, SLOs, and error alerts — instead of just monitor checks as before. Windows also gained an indefinite type: leave the end date empty for a window with no end, useful for maintenance of unknown duration. Data collection and incident tracking are unaffected either way; only outbound notifications are held.
