...
- The instance serves the installer and the binaries at two new public routes, `GET /install.sh` and `GET /agent/{file}` — unauthenticated (installation would be impossible otherwise) and behind their own rate limiters; if a reverse proxy with a path allowlist sits in front of the instance, add these two to it.
