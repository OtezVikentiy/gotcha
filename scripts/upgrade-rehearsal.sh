...
cmd_rollback() {
  ...
  while [ "$i" -lt 60 ]; do
    ...
    # "restarting" не менее верный признак отказа, чем "exited": restart:unless-stopped
    # перезапускает упавший процесс раньше, чем опрос раз в секунду успевает застать
    # "exited". Настоящее зависание состояние не меняет (unhealthy не запускает рестарт) —
    # "running" 60 раз подряд надёжно отличает его от отказа.
