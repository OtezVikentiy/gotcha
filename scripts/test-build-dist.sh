#!/usr/bin/env bash
# Тест раскладки тарбола поставки: собирает через build-dist.sh под
# архитектуру ХОСТА (иначе gotcha-бинарь из тарбола нельзя было бы
# запустить, чтобы проверить версию) и проверяет состав списком целиком.
set -uo pipefail
