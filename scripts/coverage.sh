...
# бралось от замера 39.5%; с тех пор покрытие точки входа выросло до 42.9%, а пол
# остался прежним — то есть храповик четыре с лишним пункта не держал ничего и
# допускал бесшумный откат. Подтянут к факту с допуском 0.4пп на плавание метрики;
# двигать вверх, как и остальные.
CMD_FLOOR=42.5
