...
#
# Теперь считается только рукописный код. Первое значение (80.5) бралось от
# замера 80.9%; к концу волны устранения аудита факт вырос до 83.6%, и разрыв
# в три пункта снова перестал что-либо держать — подтянут к факту. Допуск
# небольшой, потому что интегральное покрытие слегка плавает от прогона к
# прогону: часть строк исполняется только при определённом порядке
