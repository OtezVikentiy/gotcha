#!/usr/bin/env bash
# Собирает тарбол поставки для bare-metal установки: серверный бинарь под
# запрошенную архитектуру, оба агентских бинаря (парк хостов бывает смешанный
# по архитектуре независимо от --arch тарбола), конфиги ClickHouse и
# install-bare-metal.sh. Флаги ldflags и порядок сборки повторяют Dockerfile.
set -euo pipefail
