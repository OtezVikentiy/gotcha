...
// TouchEntry — один хост, встреченный на приёме: имя, версия агента, если
// экспорт пришёл от internal/agent (Task 9+, A2) и версию удалось извлечь, и
// метки environment/role (B1), если их удалось извлечь из ресурсных атрибутов.
//
// AgentVersion, Environment, Role — string, не *string, как в спеке §3.2:
// пустая строка несёт смысл «значение неизвестно ИЗ ЭТОГО батча» и трактуется
// как «не менять существующее значение» — тот же смысл, что и *string==nil,
// но без указателя на вызывающей стороне (ingest собирает TouchEntry в цикле
// по точкам метрик, где для OTel-коллектора и SDK-транзакций части полей
// попросту нет, — плодить nil-проверки под это было бы накладнее, чем просто
// сравнивать с ""). NULLIF/CASE на стороне SQL (host.go, Store.Upsert) даёт
// тот же эффект, что дал бы *string == nil — отклонение формы, не смысла
// контракта.
