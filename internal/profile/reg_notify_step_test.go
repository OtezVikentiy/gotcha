...
func TestRegressionNotifierStepDispatchesOnlyToChannelSet(t *testing.T) {
	...
	if err := n.NotifyStep(ctx, r.ID, []int64{c1}, 2); err != nil {
	...
	if err := pool.QueryRow(ctx,
		"SELECT count(*) FROM incident_escalations WHERE incident_source='profile' AND incident_id=$1 AND channel_id=$2 AND step=2",
		r.ID, c1).Scan(&count); err != nil {
	...
	if err := pool.QueryRow(ctx,
		"SELECT count(*) FROM incident_escalations WHERE incident_source='profile' AND incident_id=$1 AND channel_id=$2",
		r.ID, c2).Scan(&count); err != nil {
...
func TestRegressionNotifierStepNilChannelIDsSendsToAllDeliverable(t *testing.T) {
	...
	if err := n.NotifyStep(ctx, r.ID, nil, 0); err != nil {
		t.Fatalf("NotifyStep: %v", err)
	}
	...
	var count int
	if err := pool.QueryRow(ctx,
		"SELECT count(*) FROM incident_escalations WHERE incident_source='profile' AND incident_id=$1 AND step=0",
