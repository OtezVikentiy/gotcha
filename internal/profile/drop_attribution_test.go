...
// Профиль без project_id (0) не атрибутируется в сток; переполнение буфера —
...
func TestWriterAttributesDropsPerProject(t *testing.T) {
	...
	now := time.Now().UTC()
	sample := func(name string) Profile {
		return Profile{Type: "cpu", Timestamp: now, Samples: []Sample{
			{Stack: []Frame{{Function: name}}, Value: 1},
		}}
	}
	// Заполняем buf (oldest→newest): project1, project0 (без атрибуции), project2.
	for i, p := range []int64{1, 0, 2} {
		w.Add(p, sample(string(rune('a'+i))))
	}
	// Три новых профиля project3 выбивают три самых старых: project1, project0
	// (не атрибутируется), project2.
	for i := 0; i < 3; i++ {
		w.Add(3, sample(string(rune('x'+i))))
	}
