...
func FuzzScrub(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, in string) {
		...
		// marker раньше был константой "S3CRET". Находка №114: случай "URL среди
		// фаззового текста" дописывает СЫРЫЙ фаззовый `in` следом за
		// вставленным секретом, и фазз рано или поздно генерирует `in`,
		// САМ содержащий подстроку "S3CRET" в постороннем месте (не после
		// token=) — воспроизведено на in="xS3CRETx"
		// (testdata/fuzz/FuzzScrub/feb3e23aaefeaa26, см. task-9-report.md):
		// падение было "секрет пережил скраб", хотя единственный
		// настоящий секрет (тот, что после token=) скраб отредактировал
		// как положено, а в выходе остался ЧУЖОЙ, случайно совпавший кусок
		// фаззового мусора. Проверка ловила совпадение с собственным же
		// тестом, а не дефект скрубера.
		//
		// Маркер строится длиннее `in` (в байтах — strings.Contains ищет
		// байтовую подстроку, а `in` может быть невалидным UTF-8, который
		// фаззер умеет генерировать, поэтому важна именно len(), а не
		// рунная длина): подстрока не может быть длиннее строки, в которой
		// её ищут, — значит marker физически не может целиком встретиться
		// внутри `in`, каким бы ни был его контент. Гарантия по построению
		// (следует из самого определения подстроки), а не по вероятности —
		// в отличие, скажем, от хэша фиксированной длины, где совпадение не
		// невозможно, а лишь маловероятно, а фазз как раз охотится на
		// маловероятное.
