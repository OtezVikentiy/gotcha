...
Проверки ставит в очередь планировщик аптайма — он поднимается в режимах `--mode=web`, `--mode=uptime` и `--mode=all` (в `--mode=ingest` его нет; см. [«Режимы процесса»](/docs/configuration#rezhimy-processa-chto-zapuskaet-kazhdyy-mode)), а выполняет их либо встроенный исполнитель (только `--mode=uptime` и `--mode=all`), либо [выносная проба](/docs/probes) в другом регионе.
