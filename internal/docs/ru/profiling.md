...
Успешный приём отвечает `202 Accepted`. Если профили выключены на инстансе, запись молча пропускается, но ответ всё равно `202`. Если квота организации на профили исчерпана, приём отвечает `429`. Отдельно от квоты — переполнение буфера записи в ClickHouse: приём отвечает `503` с заголовком `Retry-After`, тело не принято вовсе, повтор дублей не создаст (см. [Мониторинг самого gotcha](/docs/self-monitoring) — причина `overloaded`); квоту такой отказ не расходует — проверка перегрузки идёт раньше её списания, так что повтор безопасен и с этой стороны.
...
> `Link; rel="deprecation"`, и в 2.0 он будет удалён. Переведите отправителей
...
Раздел открывается по ссылке **«Профили»** в подменю «Производительность» — `/projects/<id>/profiles`. Сверху — общий контрол [окна времени](/docs/time-range) (пресеты 1ч / 24ч / 7д / 30д или произвольный диапазон, по умолчанию 24ч). Таблица группирует профили по (сервис, тип, транзакция) и показывает число сэмплов в каждой группе; клик по строке открывает флеймограф этой группы за выбранный период. Сверху есть ссылка на **«Регрессии профилей»** (см. ниже).
