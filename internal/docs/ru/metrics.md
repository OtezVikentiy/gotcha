...
Точка со временем старше 90 дней в прошлом относительно момента приёма отбрасывается (та же защита партиций ClickHouse, что и у событий/трасс). Точка со временем из будущего не отбрасывается, а приводится к моменту приёма: графики и пороговые правила читают данные только до текущего времени сервера, и точка «из будущего» была бы для них потеряна. Такие точки считаются в `gotcha_metric_points_clock_skew_total` (см. [Самонаблюдение](/docs/self-monitoring)) — растущий счётчик означает, что часы экспортёра спешат.
...
Системные метрики хоста (имена вида `system.*` — CPU, память, диск, сеть, load average с сервера) в этом общем списке по умолчанию скрыты за переключателем «Системные метрики»: их проще смотреть по хосту, с готовыми графиками и встроенными порогами, — см. [Хосты](/docs/hosts).

## Детальная страница метрики

Клик по имени метрики открывает `/projects/{id}/metrics/{name}` — график временного ряда с фильтрами:

- **Период** — `1h` / `24h` / `7d` / `30d` или произвольный диапазон (контрол [окна времени](/docs/time-range)); шаг корзины подстраивается под ~120 точек — около минуты на `1h`, ~12 минут на `24h`, ~1.4 часа на `7d`, ~6 часов на `30d`;
...
Отдельной страницы настроек метрик на уровне проекта (в отличие, например, от ретенции спанов в «Производительности») пока нет — метрики хранятся по общей политике инстанса. Приём метрик считается в месячную квоту организации; лимит по умолчанию задаётся оператором через `GOTCHA_DEFAULT_METRIC_QUOTA` (см. [Конфигурацию](/docs/configuration)) и настраивается тонко в «Настройках организации → Использование и лимиты». При исчерпании квоты `/v1/metrics` отвечает `429`, уже принятые точки не удаляются. Отдельно от квоты и per-DSN rate limit — переполнение буфера записи в ClickHouse: `/v1/metrics` отвечает `503` с заголовком `Retry-After`, ни одна точка из тела не принята, повтор дублей не создаст (см. [Мониторинг самого gotcha](/docs/self-monitoring) — причина `overloaded`); квоту такой отказ не расходует — проверка перегрузки идёт раньше её списания, так что повтор безопасен и с этой стороны.
