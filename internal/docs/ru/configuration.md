...
|---|---|---|
| `GOTCHA_LISTEN_ADDR` | `:8080` | Адрес и порт, который слушает HTTP-сервер **внутри контейнера**. Как правило, менять не нужно — наружу порт пробрасывается через `docker-compose.yml`/`GOTCHA_COMPOSE_PORT` (см. [Установку](/docs/installation)), а не через эту переменную. |
