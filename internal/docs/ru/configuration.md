...
|---|---|---|
| `GOTCHA_LISTEN_ADDR` | `:8080` | Адрес и порт, который слушает HTTP-сервер **внутри контейнера**. Как правило, менять не нужно — наружу порт пробрасывается через `docker-compose.yml`/`GOTCHA_COMPOSE_PORT` (см. [Установку](/docs/installation)), а не через эту переменную. На bare-metal контейнера нет вовсе, и это просто адрес, который слушает сам процесс на хосте: `install-bare-metal.sh` явно прописывает `127.0.0.1:8080`, а наружу порт публикует установленный им же nginx (см. [Установку без Docker](/docs/installation-bare-metal)). |
...
сам `GOTCHA_LISTEN_ADDR` в `gotcha.env` (по умолчанию `127.0.0.1:8080`, наружу порт
публикует установленный инсталлятором nginx, см. [Установку без
