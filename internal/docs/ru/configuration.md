...
**Отталкиваться от симптома, а не от чисел.** Docker даёт сети контейнеров MTU 1500, не глядя на аплинк хоста, а у VPS за туннелем (GRE, VXLAN, OpenVZ) он сплошь и рядом 1450. **Само по себе это расхождение нормально и почти всегда безвредно.** Когда узкое звено — собственный интерфейс хоста, ядро сообщает контейнеру, что путь у́же, тот понижает MTU маршрута, и соединение подстраивается само. Именно поэтому бесчисленные установки живут с аплинком 1450 и контейнерами 1500, ничего не замечая.

Проблемой это становится, только если узкое звено где-то на пути *и* ICMP-сообщения, которые о нём докладывают, кто-то режет. Тогда соединению неоткуда узнать про ограничение, и крупные пакеты пропадают. Мелкие обмены при этом идут, поэтому инстанс выглядит здоровым — до первого крупного. TLS-хендшейк как раз крупный, отсюда и симптом: таймаут на заведомо живом соединении — TCP установлен, приветствие SMTP получено, дальше тишина.

То есть признаком служит **пара**: исходящее TLS падает по таймауту при том, что мелкие обмены по тому же соединению проходят, **и** внешний интерфейс хоста меньше 1500. Порознь ни то, ни другое ни о чём не говорит.
...
Если у внешнего интерфейса (`ens3`, `eth0`) MTU меньше 1500, у `docker0` — 1500, **и** при этом наблюдается описанный таймаут, задайте `GOTCHA_NET_MTU` равным значению интерфейса и поднимите стек заново:
