...
| `GOTCHA_NET_MTU` | `1500` | MTU сети контейнеров. Задайте равным MTU внешнего интерфейса хоста, если тот меньше 1500. |

**Когда MTU имеет значение.** Docker даёт сети контейнеров MTU 1500, не глядя на аплинк хоста, а у VPS за туннелем (GRE, VXLAN, OpenVZ) он сплошь и рядом 1450. Контейнер анонсирует MSS, который тракт не вывозит: сервер отвечает сегментами, которые не пролезают, и они пропадают без предупреждения. Мелкие обмены при этом работают, поэтому инстанс выглядит здоровым — до первого крупного. TLS-хендшейк как раз крупный, отсюда и симптом: таймаут на заведомо живом соединении — TCP установлен, приветствие SMTP получено, дальше тишина.

Этим же путём идёт всё исходящее — почта, вебхуки, OAuth и HTTP-проверки аптайма, — поэтому наблюдаемый сайт может числиться недоступным из-за MTU контейнера, который за ним наблюдает.
...
Если у внешнего интерфейса (`ens3`, `eth0`) MTU меньше 1500, а у `docker0` — 1500, задайте `GOTCHA_NET_MTU` равным значению интерфейса и выполните `docker compose up -d`. Compose пересоздаст сеть и перезапустит контейнеры сам, ручной `down` не нужен.
