...
|---|---|---|
| `GOTCHA_LISTEN_ADDR` | `:8080` | Адрес и порт, который слушает HTTP-сервер **внутри контейнера**. Как правило, менять не нужно — наружу порт пробрасывается через `docker-compose.yml`/`GOTCHA_PORT` (см. [Установку](/docs/installation)), а не через эту переменную. |
...
| `GOTCHA_SECRET_KEY_PREV` | *(пусто)* | Предыдущий мастер-ключ на время ротации `GOTCHA_SECRET_KEY` — задаётся вместе с новым `GOTCHA_SECRET_KEY` на время перехода, затем убирается. Пока задан, приложение расшифровывает и им, и текущим ключом, а шифрует только текущим. Приложение отказывается стартовать при бессмысленной паре ключей: `GOTCHA_SECRET_KEY_PREV` равен текущему `GOTCHA_SECRET_KEY`, любой из них равен дефолтному dev-ключу при заданном `GOTCHA_SECRET_KEY_PREV`. `GOTCHA_SECRET_KEY_ALLOW_INSECURE=1` эти проверки не снимает — это не про стойкость ключа, а про конфиг, который физически не может сделать то, что от него ждут. Процедура ротации — в [«Приватность и 152-ФЗ»](/docs/privacy). |
