...
| `GOTCHA_MAX_BUFFER_BYTES` | авто (см. ниже) | Байтовый потолок КАЖДОГО буфера писателя в ClickHouse (события, спаны, метрики, профили). Буферы растут, пока ClickHouse недоступен, — так телеметрия не теряется на коротком сбое; потолок ограничивает эту плату. Заданное здесь число всегда побеждает авто-поведение. Результаты аптайм-проверок буферизуются отдельно и ограничены по числу строк (10000), а не этой переменной. |
...
| `GOTCHA_AGENT_DIST_RATE_PER_MIN` | `120` | Порог per-IP лимитера `GET /agent/{file}` (загрузка бинаря агента и `SHA256SUMS`). Одна установка/обновление стоит 2 запроса, поэтому дефолт даёт ~60 хостов в минуту с одного IP — с запасом для массовой раскатки (Ansible/Terraform) или обновления парка за одним NAT/egress-адресом. Поднимите значение, если за одним IP стоит парк больше. |
