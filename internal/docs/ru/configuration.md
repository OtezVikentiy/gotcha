...
| `GOTCHA_BASE_URL` | `http://localhost:8080` | Публичный адрес вашего инстанса — то, как до него реально добираются пользователи и SDK. Используется для построения DSN проектов, ссылок в письмах-приглашениях и ссылок на инциденты в алертах (Telegram/webhook/email). **Должен точно совпадать** со схемой+хостом+портом, по которым инстанс реально доступен. Если значение не `localhost`/`127.0.0.1`, приложение в режимах `web`/`all` требует **не-дефолтный** [`GOTCHA_SECRET_KEY`](#security) — см. ниже. Если значение не начинается с `https://` и не локальное — в лог пишется предупреждение (сессионные cookie идут открытым текстом). |
