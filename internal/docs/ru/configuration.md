...
| `GOTCHA_COMPOSE_BIND` | `127.0.0.1` | Адрес хоста, на который публикуется порт приложения. По умолчанию — только loopback: снаружи сервера порт недоступен, пока вы явно не зададите `0.0.0.0` (см. [Установку](/docs/installation)). |
