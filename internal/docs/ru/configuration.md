...
| Переменная | По умолчанию | Описание |
|---|---|---|
| `GOTCHA_LOG_LEVEL` | `info` | Детализация логов: `debug`, `info`, `warn` (алиас `warning`) или `error`, без учёта регистра. Поднимите до `debug`, чтобы получить больше подробностей во время инцидента, не пересобирая образ. Нераспознанное значение — отказ старта, а не тихий откат к `info`: опечатку в конфигурации логов (например, `trace`) нужно заметить сразу, а не искать в момент, когда детализации не хватает в разгар инцидента. |
| `GOTCHA_LOG_FORMAT` | `text` | Формат логов: `text` или `json`, без учёта регистра. `json` удобнее, когда логи уезжают в Loki/ELK. Нераспознанное значение — отказ старта. |
