...
## Как задать переменные окружения на bare-metal

На установке без Docker ([Установка без Docker](/docs/installation-bare-metal)) переменные живут не в `.env` рядом с `docker-compose.yml`, а в файле окружения самого systemd-юнита — `/etc/gotcha/gotcha.env`:
...
Файл создаётся инсталлятором с владельцем `root:gotcha` и правами `0640` — сохраните их после ручной правки (`chown root:gotcha /etc/gotcha/gotcha.env && chmod 0640 /etc/gotcha/gotcha.env`), файл хранит пароли обеих баз и мастер-ключ шифрования. Второй способ, равнозначный блоку `environment:` у Docker Compose, — переопределение переменной прямо в юните (`systemctl edit gotcha`, секция `[Service]`, `Environment=ИМЯ=значение`); для большинства правок проще отредактировать сам `gotcha.env`.
...
|---|---|---|
| `GOTCHA_LISTEN_ADDR` | `:8080` | Адрес и порт, который слушает HTTP-сервер **внутри контейнера**. Как правило, менять не нужно — наружу порт пробрасывается через `docker-compose.yml`/`GOTCHA_COMPOSE_PORT` (см. [Установку](/docs/installation)), а не через эту переменную. На bare-metal контейнера нет вовсе, и это просто адрес, который слушает сам процесс на хосте: `install-bare-metal.sh` явно прописывает `127.0.0.1:8080`, а наружу его выводит ваш обратный прокси (см. «Внешний доступ и TLS» в [Установке без Docker](/docs/installation-bare-metal)). |
...
| `GOMEMLIMIT` | *(вычисляется из cgroup)* | Стандартная (без префикса `GOTCHA_`) переменная самого Go-рантайма — потолок кучи. Оставленная незаданной, читается напрямую `internal/memlimit`, который в контейнере сам выводит её из потолка cgroup (см. «Переменные только для Compose» ниже, `GOTCHA_COMPOSE_MEM_LIMIT`); задавайте вручную только на bare metal без cgroup или чтобы переопределить выведенное значение. На bare-metal-установке эту сборку делает сам `install-bare-metal.sh`: вычисляет значение из флага `--mem-limit` (по умолчанию 819MiB — 80% от `MemoryMax=1024M` юнита) и пишет прямо в `gotcha.env`; руками задавать нужно, только если меняете лимит без переустановки. |
...
Эти четыре — **переменные подстановки Docker Compose**, а не конфигурация процесса gotcha: приложение их не читает. Compose подставляет их в настройки контейнеров баз и в DSN выше. На bare-metal этого слоя нет вовсе: `install-bare-metal.sh` генерирует пароли обеих баз сам (`openssl rand -hex 24`) и сразу кладёт их в `GOTCHA_PG_DSN`/`GOTCHA_CH_DSN` внутри `/etc/gotcha/gotcha.env` — отдельных переменных для паролей там нет.
...
| `GOTCHA_COMPOSE_BIND` | `127.0.0.1` | Адрес хоста, на который публикуется порт приложения. По умолчанию — только loopback: снаружи сервера порт недоступен, пока вы явно не зададите `0.0.0.0` (см. [Установку](/docs/installation)). Само приложение эту переменную не видит и не может понять, публикуется ли его порт наружу — держите ручки `/metrics`, `/version`, `/healthz`, `/readyz` за реверс-прокси или файрволом, см. [Усиление установки](/docs/hardening). |
...
| `GOTCHA_COMPOSE_NO_NEW_PRIVS` | `true` | Флаг `no-new-privileges` у контейнера приложения. Ставить `false` осмысленно в единственном случае: контейнер падает на старте с `exec /usr/local/bin/gotcha: operation not permitted`, а `postgres` и `clickhouse` при этом здоровы. Это отказ AppArmor на хосте, где сам `dockerd` работает под профилем (Docker из snap), а не поломка образа — разбор и проверка в разделе «Устранение неполадок» [Установки](/docs/installation). Правильное лечение — Docker не из snap; переключатель даёт запуститься до этого. Остальная изоляция контейнера (непривилегированный пользователь, `cap_drop: ALL`, `read_only`) не меняется. |

На bare-metal этой прослойки нет вовсе — переменные `GOTCHA_COMPOSE_*` подставляет
исключительно Docker Compose, `install-bare-metal.sh` их не читает и не пишет. Прямые
аналоги: потолок памяти задаёт флаг `--mem-limit` инсталлятора (согласованно правит
`MemoryMax` юнита и `GOMEMLIMIT` в `gotcha.env`, см. выше); адрес и порт слушателя — это
сам `GOTCHA_LISTEN_ADDR` в `gotcha.env` (по умолчанию `127.0.0.1:8080`, наружу его
выводит ваш обратный прокси, см. [Установку без
Docker](/docs/installation-bare-metal)), а не отдельные переменные бинда и порта; MTU —
свойство сетевого интерфейса хоста напрямую, без слоя контейнерной сети, трогать нечего;
`NoNewPrivileges=yes` в юните безусловен — ни переключателя, ни оговорки про
AppArmor/snap, которая существует только для Docker.
...
На bare-metal образ не собирается вовсе — версию и коммит в бинарь зашивает
`scripts/build-dist.sh` при сборке релизного тарбола; этих трёх переменных
bare-metal-инсталляция не касается.
...
Изменение retention применяется на следующем старте приложения (значение используется, чтобы выставить TTL на таблицах ClickHouse) — задним числом уже удалённые данные не восстановятся. TTL — свойство самих таблиц ClickHouse, а не отдельной реплики: держите каждую переменную `*_RETENTION_DAYS` одинаковой на всех репликах, работающих с одной парой PostgreSQL/ClickHouse — расхождение заставляет их перебивать TTL друг у друга на каждом старте с `GOTCHA_AUTO_MIGRATE_ENABLED=true`. Действующие значения (последними записанные той репликой, которая их применила — при выключенной автомиграции они могут отличаться от `.env` этой конкретной реплики) видны как `gotcha_retention_days{dataset="…"}` — см. [Мониторинг gotcha изнутри](/docs/self-monitoring).
...
| `GOTCHA_INGEST_RATE_PER_SEC` | `500` | Per-DSN лимит приёма: запросов в секунду на проект (токен-бакет, burst = 2×лимит). Проверяется после аутентификации DSN и до квот; сверх лимита API отвечает `429` с коротким `Retry-After`. `0` отключает лимит. Бакет живёт в памяти процесса приёма, отдельно на каждой реплике `--mode=ingest`/`all` — за балансировщиком из N таких реплик фактический потолок на проект доходит до N×значения переменной, а не равен ему. |
| `GOTCHA_INGEST_PREAUTH_RATE_PER_SEC` | `2000` | Лимит по клиентскому IP, проверяется до аутентификации DSN на каждом маршруте приёма (токен-бакет, burst = 2×лимит). Нарочно выше `GOTCHA_INGEST_RATE_PER_SEC`: один IP законно обслуживает несколько DSN (общий сервер приложений на несколько проектов). `0` отключает лимит. |
| `GOTCHA_INGEST_SIGNAL_RATE_PER_SEC` | `2` | Лимит по клиентскому IP на запись сигнала о неверном ключе (токен-бакет, burst = 5×лимит) — много туже `GOTCHA_INGEST_PREAUTH_RATE_PER_SEC`: легитимный несовпадающий DSN повторно бьёт в свой же проект, а не перебирает чужие, и редкие касания не должны упираться в лимит. `0` отключает лимит. |
...
| `GOTCHA_CARDINALITY_LIMIT` | `10000` | Потолок РАЗЛИЧНЫХ значений открытых полей (имя транзакции, окружение, имя метрики, сервис, операция спана) на проект за окно. Значения сверх потолка не отбрасываются, а группируются под `<cardinality-limit>`; на затронутой странице показывается, по какому полю сработал потолок, и примеры сгруппированных значений. Причина почти всегда — идентификатор, попавший в имя, см. [Кардинальность](/docs/cardinality). `0` снимает ограничение. Счётчик различённых значений — тоже в памяти каждой реплики приёма отдельно, без синхронизации между ними: одно и то же значение может схлопнуться в `<cardinality-limit>` на одной реплике и пройти как есть на другой — в ClickHouse окажутся обе версии одного ряда, как будто это два разных значения. |
...
| `GOTCHA_ESCALATION_INTERVAL_SECONDS` | `60` | Как часто (в секундах) планировщик эскалаций проверяет открытые неподтверждённые инциденты и продвигает [лесенку](/docs/escalations) на очередную ступень. Минимум — 1 секунда. Гейтится тем же `GOTCHA_EVALUATORS_ENABLED`, что и пять циклов выше. |
...
| `GOTCHA_AUTO_MIGRATE_ENABLED` | `true` | Применять миграции схемы БД автоматически при старте. `false` — миграции нужно применить отдельным шагом заранее, иначе приложение откажется стартовать на устаревшей схеме. Подробности и когда это нужно — в [Обновлении](/docs/upgrade). |
...
| `GOTCHA_TRUSTED_RECIPIENTS` | пусто | Домены и хосты вашего контура через запятую: почта на этих доменах и вебхуки на этих хостах получают детали события даже при выключенном `GOTCHA_EXTERNAL_CHANNEL_DETAILS_ENABLED`. Совпадение по границе доменной метки (`corp.example` покрывает `mail.corp.example`, но не `evilcorp.example`). Хост инстанса из `GOTCHA_BASE_URL` и адреса внутренней сети доверенные всегда и без настройки. См. [Приватность и 152-ФЗ](/docs/privacy). |

> Флаг командной строки `--migrate-only` применяет схему и завершает процесс, не поднимая ни одного компонента: init-job для развёртываний с `GOTCHA_AUTO_MIGRATE_ENABLED=false`. См. [Обновление](/docs/upgrade).

> `GOTCHA_AUTO_MIGRATE_ENABLED=false` — единственная конфигурация, которая переживает переход primary PostgreSQL в режим только чтения (failover или окно обслуживания managed-БД): сам старт не делает в PostgreSQL ни одной записи и проходит успешно. Но работающий инстанс в этом состоянии лишь частично пригоден: страницы, которые читают телеметрию и настройки, работают, а **вход и регистрация — нет**: создание сессии требует записи в PostgreSQL, а база её отвергает (сбой при этом логируется, не молчит — но посетитель всё равно видит голую страницу ошибки). Со включённой автомиграцией (по умолчанию) старт на read-only primary откажет сразу — это ожидаемо: применение миграций требует доступа на запись.

## Режимы процесса: что запускает каждый `--mode=`

Роль процесса задаётся флагом бинаря `--mode=` (не переменной окружения): `all` по умолчанию — так работает штатный `docker-compose.yml`; `web`, `ingest` и `uptime` — для раздельного развёртывания на несколько реплик; `probe` — выносная проба (см. ниже). Таблица показывает, какие компоненты поднимает каждый режим. Все четыре режима с базой держат HTTP-слушатель со служебными ручками (`/healthz`, `/readyz`, `/version`, `/metrics`, см. [Мониторинг самого gotcha](/docs/self-monitoring)) и на старте выполняют бэкфилл секретов под текущий `GOTCHA_SECRET_KEY`.
...
Отсюда два следствия для раздельного развёртывания. Во-первых, `web` — не «только интерфейс»: он доставляет уведомления и подчищает базу наравне с остальными, поэтому запись `notify outbox janitor` в его логе — норма, а не признак перепутанного режима. Во-вторых, без реплики `uptime` или `all` оценщики не работают нигде — поднимите их явно через `GOTCHA_EVALUATORS_ENABLED=true` ровно на одной реплике (см. [«Ограничители и оценщики»](#ogranichiteli-i-ocenschiki)).

`web` и `ingest` безопасно масштабировать на несколько реплик: они не координируются между собой, но и не дублируют работу друг друга. Внутри `uptime`/`all` планировщик и исполнитель аптайм-проверок тоже безопасны на нескольких репликах — постановка в очередь идемпотентна (`internal/uptime/queue.go`: `FOR UPDATE OF m SKIP LOCKED` и `ON CONFLICT (monitor_id, region) DO NOTHING`), исполнение честно делит очередь между репликами (`FOR UPDATE OF q SKIP LOCKED`); watchdog — туда же, пропуски heartbeat, SSL-алерты и напоминания клеймятся атомарно (`internal/uptime/watchdog.go`, методы `Claim*`) — лишняя реплика добавляет пропускную способность, а не задваивает пейджи. **Единственности по-прежнему требуют шесть оценщиков** (регрессии производительности, правила по метрикам, регрессии профилей, пороги хостов, SLO burn-rate, планировщик эскалаций — см. [«Ограничители и оценщики»](#ogranichiteli-i-ocenschiki)): каждый тикает независимо на каждой реплике безо всякой координации между ними. **Поэтому `uptime` и `all` всё равно обязаны запускаться ровно одной репликой каждый** — вторая реплика не добавляет резерва, а даёт два параллельных прохода по одним и тем же правилам и порогам на каждый тик.
...
| `GOTCHA_OIDC_TRUST_EMAIL` | `false` | Доверять ли этому OIDC-провайдеру заявленный им `email`/`email_verified` настолько, чтобы по нему автоматически заводить новый аккаунт (открытая регистрация или приглашение) или привязывать вход к существующему по совпадению email. Дефолт — `false`: без этой переменной вход через OIDC работает только для уже привязанных аккаунтов (см. [SSO](/docs/sso)). Включайте `true` только для собственного одно-тенантного IdP (свой Keycloak/Authentik/Zitadel и т.п.), где директорией управляете вы сами. **Не включайте** для публичного мульти-тенантного IdP (тот же Google/сторонний Auth0-тенант общего назначения), где произвольный человек может завести директорию и указать чужой email как свой — это открывает угон парольного аккаунта чужим адресом. |
...
- [Установка без Docker](/docs/installation-bare-metal) — тот же контракт переменных, другой способ их задать.
