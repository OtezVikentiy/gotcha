...
браузеров на месяцы вперёд, и снять его оттуда — вопрос месяцев, а не минут. Единственный
выход при аварии — `GOTCHA_HSTS_MAX_AGE_SECONDS=0`, дождаться, пока пин истечёт у уже
посетивших инстанс клиентов, и только потом выключать заголовок целиком. Выключенный HSTS
пин сам по себе **не снимает** — он лишь перестаёт продлеваться.
