...
На bare-metal закройте `/metrics` и `/version` в своём прокси — примеры в разделе
«Внешний доступ и TLS» [Установки без Docker](/docs/installation-bare-metal) это уже
делают: снаружи обе ручки отвечают 403 (в примере nginx открыт только loopback,
`127.0.0.1`/`::1`; примеры Apache и Caddy закрывают их для всех без исключений).
`/healthz` и `/readyz` умышленно оставлены открытыми — это единственные две ручки, которым нужно
...
же `location`-блок вашего сайта nginx (`location ~ ^/(metrics|version|healthz|readyz)$`) и
пустите внешнюю проверку доступности на обычную страницу вместо пробы (для Apache/Caddy —
аналогичное правило вашего прокси). Чтобы пустить к `/metrics` свой сборщик метрик,
добавьте его подсеть строкой `allow ...;` перед `deny all;` в блоке
`location ~ ^/(metrics|version)$` вашего сайта nginx и перезагрузите конфиг
