...
затронутые каналы алертов будут пропускаться (с ошибкой в логе), а SSO
организации перестанет работать. В момент восстановления вас об этом ничто не
предупредит: строки на месте, просто нечитаемы.
