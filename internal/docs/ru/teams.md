...
**Восстановление**, если доступ к аккаунту администратора инстанса утрачен: удалить аккаунт администратора всё равно нельзя, пока на инстансе есть другие пользователи, — а вот пароль восстановить можно, если только он и был утрачен.

Если забыт только пароль, два пути:

- **Самостоятельно**, когда на инстансе настроена отправка почты (`GOTCHA_SMTP_*`): страница `/forgot-password` («Забыли пароль?» на `/login`) отправляет на указанный адрес ссылку для установки нового пароля. Ссылка действует час и одноразовая; при успешной установке нового пароля все существующие сессии этого пользователя завершаются.
- **Через оператора**, когда почта не настроена (страница `/forgot-password` в этом случае честно сообщает о недоступности самостоятельного восстановления, а не показывает нерабочую форму) или доступа к почтовому ящику тоже нет: подкоманда бинаря `gotcha set-password --email=<адрес>` задаёт новый пароль напрямую в БД. Пароль читается из стандартного ввода, а не аргументом командной строки — аргумент осел бы в истории оболочки и в списке процессов. Не набирайте пароль как литерал в команде (это тоже попадёт в историю) — используйте `read -s`, чтобы он не отображался на экране и не сохранился в истории:

  ```bash
  read -rs -p 'Новый пароль: ' PW && printf '%s\n' "$PW" | gotcha set-password --email=admin@example.com; unset PW
  ```

  Сама подкоманда ввод не маскирует — если набирать пароль напрямую в её собственном приглашении, минуя `read -s`, символы будут видны на экране.

Если доступа к аккаунту нет совсем (пароль ни восстановить, ни сбросить некому — например, утрачен и сам email), оператор с доступом к БД инстанса назначает нового администратора напрямую, одной транзакцией — `BEGIN`/`COMMIT` обязательны: `psql` по умолчанию в autocommit, и без них опечатка в email после первого `UPDATE` молча оставит инстанс без администратора вовсе:
...
У owner'а или admin'а нет кнопки, которая напрямую завершает активную сессию
другого участника, — выход с других устройств доступен только самому
пользователю, из раздела «Сессии» на `/profile` («Выйти со всех других
устройств» завершает все сессии этого аккаунта, кроме той, с которой
отправлен сам запрос).

Если устройство участника утеряно или скомпрометировано, а действовать нужно
без его участия, реально завершает все его сессии смена пароля этого
аккаунта: она немедленно удаляет все строки сессий пользователя — тот же
механизм, на котором держатся пути восстановления выше. Участник может
сделать это сам, оказавшись за доверенным устройством; если не может,
оператор с доступом к серверу инстанса форсирует это командой `gotcha
set-password --email=<адрес>` (см. выше) — у admin или owner без доступа к
серверу такой возможности через веб-интерфейс нет.

Ещё два действия закрывают смежный, но другой риск, и их стоит выполнить заодно:

- **Отозвать ключи приёма проекта**, доступные с этого устройства, в
  настройках проекта — сессии входа это не касается, но останавливает
  отправку телеметрии ключом, сохранённым на устройстве (отзыв вступает в
  силу не сразу, до 30 секунд, см. [Ключи приёма](/docs/keys)).
- **Удалить участника из организации** («Настройки» → «Организация» →
  «Участники»), если доступ ему больше не положен вовсе: членство
  проверяется на каждый запрос, а не кешируется, так что доступ к данным
  всех проектов пропадает немедленно — даже если его сессия входа всё ещё
  действительна, он лишь увидит аккаунт без единой организации.
...
Дальнейшее поведение регистрации по приглашению зависит от **режима регистрации** инсталляции (`GOTCHA_REGISTRATION_MODE`, см. [Конфигурация](/docs/configuration)):
...
| Правила алертов (новая проблема / регрессия / всплеск) | — | ✓ | ✓ | ✓ |
| [Эскалации](/docs/escalations): лесенки уведомлений по критичности («Критично»/«Предупреждение») | — | ✓ | ✓ | ✓ |
| [Подавление шторма](/docs/alert-suppression): рёбра зависимостей — создание, правка, удаление | — | ✓ | ✓ | ✓ |
...
| [Оповещения по метрикам](/docs/metric-alerts): создание, правка и выключение, удаление | — | ✓ | ✓ | ✓ |
| [SLO](/docs/slo): создание, удаление | — | ✓ | ✓ | ✓ |
| [Рецепты](/docs/recipes): включение рекомендованных порогов сервиса | — | ✓ | ✓ | ✓ |
| Хосты: пороги проекта, группы по меткам окружения/роли, переопределение порога на конкретном хосте, удаление хоста | — | ✓ | ✓ | ✓ |
| [Выгрузки](/docs/exports): постановка и удаление заявки на выгрузку | — | ✓ (видит и скачивает только свои заявки) | ✓ (все заявки проекта) | ✓ (все заявки проекта) |
