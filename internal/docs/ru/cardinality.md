...
У самой защиты тоже есть граница: она помнит значения в памяти процесса, и суммарно по всем проектам их не больше миллиона. Память — это память конкретной реплики приёма, без синхронизации между ними: при нескольких `--mode=ingest`/`all` за балансировщиком одно и то же значение может схлопнуться в `<cardinality-limit>` на одной реплике и пройти как есть на другой — тогда в хранилище окажутся сразу обе версии одного и того же логического значения, как будто это два разных. Это и есть ответ на вопрос «почему у меня в данных и схлопнутое значение, и целое одновременно». При исчерпании бюджета сначала выбрасываются наборы проектов с истёкшим окном, а затем новые значения перестают запоминаться — защита продолжает работать, но не растёт. Сколько значений она помнит прямо сейчас, показывает `gotcha_cardinality_tracked_values` в `/metrics`. Число различных ИМЁН полей на проект тоже ограничено (200): имя поля приходит от отправителя ровно так же, как значение.
