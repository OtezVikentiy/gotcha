...
**Пред-полётная проверка.** Прежде чем обновлять боевой инстанс, прогоните
новый бинарь с реальным `.env` на стенде — той же командой, которой ниже
...
следует реальное применение миграций схемы, `--migrate-only` не проверяет
БД, а подключается к ней и меняет её схему.
