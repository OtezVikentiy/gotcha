...
На установке без Docker ([Установка без Docker](/docs/installation-bare-metal)) этот шаг делает сам `install-bare-metal.sh` при каждом обнаруженном обновлении — вручную снимать бэкап нужно, только если вы используете внешние PostgreSQL/ClickHouse (`--skip-databases`) или явно передали `--no-backup`; подробности — в разделе «Обновление bare-metal-инсталляции» ниже.
...
## Что меняется при обновлении с версий до 1.2.0: приём насыщенного буфера отвечает `503`, а не `200`

До v1.2.0 приём, упёршись в переполненный внутренний буфер (очередь пайплайна,
батчер ClickHouse, буфер конкретного писателя — спанов/метрик/логов/профилей),
отвечал `200` и вытеснял из буфера самую старую ещё не записанную запись,
освобождая место новой. Начиная с v1.2.0 такой приём отвечает `503` с
заголовком `Retry-After`, не принимая присланное вовсе — вместо того, чтобы
принять и тут же потерять что-то другое. Если ваш SDK или коллектор не
уважает `Retry-After` и не повторяет `503` — часть телеметрии при пиковой
нагрузке перестанет доходить с концами вместо того, чтобы доходить частично
и с потерями по вытеснению; проверьте клиент. Новое значение
`reason=overloaded` метрики `gotcha_ingest_rejected_total` считает такие
отказы отдельно от прежних причин (превышение квоты, невалидный payload).

v1.3.0 продолжает то же изменение: запрос, из которого в очередь не встало
вообще ничего (не было места даже под часть присланного), тоже теперь
отвечает `503` вместо `200`, и повторить его безопасно — квота за него уже
не списывается (списание квоты по не влезшим в очередь элементам
возвращается автоматически; списывается только за то, что реально встало в
очередь на запись). Запрос, часть конверта которого встала в очередь,
по-прежнему отвечает `200` — повторять его небезопасно: дедупликации по
event-id нет, и повтор задвоит уже принятую часть. Заодно с v1.3.0 конверт,
целиком состоящий из профилей, при исчерпанной квоте профилей тоже отвечает
`429` вместо прежнего `200` с молчаливой потерей — у профилей теперь
собственное решение о квоте наравне с событиями и трейсами, а не только
после них.

Обновите обработку ответов приёма в интеграциях, которые проверяют код
ответа: код, ожидающий только `200`/`429`/`401`/`403`/`413`, должен также
повторять `503` после паузы `Retry-After`.
...
## Обновление bare-metal-инсталляции
...
1. снимает дамп PostgreSQL (`pg_dump`) в
   `/var/lib/gotcha/backup/postgres-<версия>-<дата>.sql.gz` перед миграциями — пропустить
   этот шаг можно только явным `--no-backup`, а с `--skip-databases` (внешние
   PostgreSQL/ClickHouse) он и так пропускается: снять дамп чужой базы скрипту нечем, это
   ответственность оператора внешней СУБД;
2. останавливает сервис `gotcha`;
3. сохраняет предыдущий бинарь в `/opt/gotcha/backup/gotcha-<версия>` — единственный путь
   отката, описанный в «Откат назад» ниже;
4. устанавливает новый бинарь и раздачу агента;
5. применяет миграции схемы тем же флагом `--migrate-only`, что и Docker-путь, только
...
отказе скрипт печатает список уже сделанных шагов и код завершения, повторный запуск с
теми же флагами идемпотентен — не переделывает уже сделанное и продолжает с отказавшего
шага.
...
На bare-metal тот же разбор — по `journalctl -u gotcha --no-pager -n 100` вместо
`docker compose logs gotcha`, а флаг передаётся тем же `systemd-run`, что и обычные
миграции:
...
1. **Откатите `.env` вместе с бинарём.** Обязательно для версий ниже v0.34.0: этот релиз переименовал семнадцать серверных переменных (раздел выше), и старый бинарь новых имён не знает — он не откажется стартовать, а тихо возьмёт дефолт по переменной, которой в `.env` для него уже нет. Самые опасные из семнадцати:
   - `GOTCHA_REGISTRATION` → `GOTCHA_REGISTRATION_MODE` — старый бинарь не видит запрет и открывает публичную регистрацию;
   - `GOTCHA_SCRUB_KEYS` → `GOTCHA_SCRUB_DENY_KEYS` — скрабинг PII в логах и событиях перестаёт действовать;
   - `GOTCHA_RETENTION_DAYS` → `GOTCHA_EVENT_RETENTION_DAYS` — ретеншен молча падает к дефолту 90 дней, и более старые данные начинают удаляться;
   - `GOTCHA_ADDR` → `GOTCHA_LISTEN_ADDR` — адрес прослушивания уезжает на дефолт.
...
2. **Откатите приложение**:
   - Docker: переключитесь на предыдущий коммит/тег и пересоберите:
     ```bash
     git checkout <предыдущий-тег-или-коммит>
     make up-rebuild
     ```
   - Bare-metal: верните сохранённый предыдущий бинарь и перезапустите сервис:
     ```bash
     systemctl stop gotcha
     cp -a /opt/gotcha/backup/gotcha-<предыдущая-версия> /usr/local/bin/gotcha
     systemctl start gotcha
     ```
     Этот файл кладёт туда сам инсталлятор перед каждым обновлением (см. «Обновление
     bare-metal-инсталляции» выше) — по одному файлу на версию, из которой обновлялись;
     старые версии в `/opt/gotcha/backup/` инсталлятор сам не чистит.
...
4. **Если старт прерван** с сообщением о несовместимой схеме — откат бинаря невозможен: **восстановите резервную копию**, снятую перед обновлением (см. [Резервное копирование и восстановление](/docs/backup-restore)), и поднимите на ней предыдущую версию.
...
На bare-metal-установке — то же самое, но без Docker Compose:
...
`systemctl status gotcha` — `active (running)`. Строка `applying migrations` в журнале без последующей ошибки означает то же самое, что и на Docker-пути.
