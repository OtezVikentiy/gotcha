...
2. Перед запуском реплик выполните миграции **один раз**, отдельным разовым запуском бинарника с `GOTCHA_AUTO_MIGRATE=true` (или вообще без переопределения — это дефолт), например:
   ...
   На практике для этого достаточно один раз запустить обычный контейнер `gotcha` с `GOTCHA_AUTO_MIGRATE=true` — миграции применяются в самом начале старта, до открытия HTTP-порта. Это верно для `ingest`, `web`, `uptime` и `all`; исключение — `--mode=probe`: проба вообще не открывает соединение с базой.
