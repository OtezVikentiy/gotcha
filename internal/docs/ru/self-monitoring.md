...
Штатный compose-файл даёт контейнеру `gotcha` healthcheck на `/readyz`, но
важно понимать, что это даёт: **`docker compose` не перезапускает unhealthy
контейнер.** Проваленная проверка только меняет метку в `docker ps` (реакция
