...
Скриптом — повторный запуск с новым адресом: `sudo bash install-bare-metal.sh --base-url https://new.example.com`. Скрипт перепишет `GOTCHA_BASE_URL` в `/etc/gotcha/gotcha.env`, не трогая пароли и ключ, и перезапустит сервис. Руками — поправить ту же строку и `systemctl restart gotcha`. В обоих случаях поправьте свой прокси (имя сервера) и выпустите сертификат на новый адрес.
...
**Вход заблокирован у всех сразу после нескольких неудачных попыток.** В `/etc/gotcha/gotcha.env` нет `GOTCHA_TRUSTED_PROXIES`, и лимитер видит всех пользователей как адрес прокси. Повторный запуск скрипта допишет строку сам; руками — `GOTCHA_TRUSTED_PROXIES=127.0.0.1/32,::1/128` и `systemctl restart gotcha`.
