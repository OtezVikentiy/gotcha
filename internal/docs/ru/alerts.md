...
Если таймаутом падает не один канал, а всё исходящее сразу — почта, вебхуки, вход через OAuth, проверки аптайма, — и при этом соединение явно устанавливается (в ошибке почты видно, что приветствие `220` получено, а споткнулось на STARTTLS), то мелкие пакеты ходят, а крупные пропадают. Такое даёт и фильтрация трафика, и — реже — MTU сети контейнеров. Начните с шагов выше, а если они ничего не дали, посмотрите `GOTCHA_NET_MTU` в разделе [Конфигурация](/docs/configuration): там описано, при каком сочетании признаков дело действительно в MTU, а при каком нет.
