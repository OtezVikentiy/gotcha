...
- **Быстрый, для первой проверки.** Добавить `GOTCHA_BIND=0.0.0.0` в `.env` и применить `docker compose up -d` — тогда порт `59080` слушает все интерфейсы, как раньше. Учтите: пока вы не поставите HTTPS-прокси, снаружи будет доступен голый HTTP-вход и открытый `/metrics` (self-телеметрия без аутентификации) с любого адреса.
...
(или `http://<IP-сервера>:59080`, если пока без домена и без HTTPS — но тогда напомним: этот адрес доступен снаружи, только если вы уже задали `GOTCHA_BIND=0.0.0.0`, как описано в шаге 4; см. чек-лист ниже, почему HTTPS важен). Примените:
...
Откройте `<GOTCHA_BASE_URL>/register` (адрес из шага 6 — домен за реверс-прокси, либо `http://<IP-сервера>:59080` с уже заданным `GOTCHA_BIND=0.0.0.0`) и зарегистрируйтесь.
...
- Сначала проверьте это: по умолчанию порт слушает только loopback (`GOTCHA_BIND=127.0.0.1`), и прямой доступ по IP сервера снаружи **не работает, пока вы не поставите реверс-прокси или не зададите `GOTCHA_BIND=0.0.0.0`** — см. шаг 4. Это не поломка, а поведение по умолчанию.
- Если `GOTCHA_BIND=0.0.0.0` уже задан (или вы используете реверс-прокси, слушающий 80/443, а не сам порт 59080), проверьте firewall на сервере. Ubuntu/Debian: `sudo ufw status` — при включённом ufw разрешите порт: `sudo ufw allow 59080/tcp`. AlmaLinux/Rocky/RHEL: firewalld включён по умолчанию — разрешите порт: `sudo firewall-cmd --permanent --add-port=59080/tcp && sudo firewall-cmd --reload`.
