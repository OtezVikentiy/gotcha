...
Не хотите ставить Docker? Есть путь без него — [Установка без Docker (bare-metal)](/docs/installation-bare-metal).
...
- **RAM.** 2 ГБ — рабочий минимум для запуска и небольшой нагрузки (личные проекты, стейджинг). «Небольшая» в цифрах — 10 000 событий, 200 000 строк логов и 5 000 трейсов по 10 спанов в сутки: такой поток на 2 ГБ идёт без единого отказа приёма, занимая в пике 1006 МБ памяти из 1962 (замер на релизе 1.6.0: 2 vCPU / 2 ГБ / 20 ГБ SSD, без свопа, с оверлеем для слабого железа — см. ниже, четыре часа непрерывной нагрузки). Для продакшена с реальным потоком событий закладывайте от 4 ГБ: под нагрузкой ClickHouse тем стабильнее, чем больше ему доступно памяти.
- **Диск.** Растёт вместе с объёмом телеметрии и сроком её хранения (retention). 20 ГБ хватает, чтобы стартовать; при заметном трафике или длинном retention планируйте больше и следите за свободным местом — как именно, см. [Мониторинг gotcha](/docs/self-monitoring). Диск обязательно SSD — и ClickHouse, и PostgreSQL чувствительны к его латентности. Порядок величины: на потоке из пункта про RAM и заводских сроках хранения (события 90 суток, спаны и метрики 30, логи 14) сами данные в ClickHouse выходят примерно на 0,7 ГБ — на небольшом сервере место занимают в основном не они, а система, образы Docker и кэш сборки.
- **Диск под сборку.** Образ приложения собирается на самом сервере, поэтому диску нужен запас сверх цифр выше: на время сборки к рабочему набору добавляются образ компилятора Go и кэш компиляции — порядка гигабайта, который после сборки уже не нужен. Освободить его: `docker builder prune -f` (удаляет только кэш сборки, образы и данные не затрагивает). Сами образы, необходимые для работы, занимают около 1,3 ГБ: ClickHouse ~780 МБ, PostgreSQL ~420 МБ, приложение ~110 МБ.
...
Замер с оверлеем на релизе 1.6.0 (2 ядра / 2 ГБ / 20 ГБ SSD, без свопа, поток из «Системных требований», четыре часа непрерывной нагрузки): ClickHouse занимает 397 МБ при отведённом ему потолке в 1 ГБ, PostgreSQL — 110 МБ, приложение — 21 МБ. Пик занятой памяти на всей машине — 1006 МБ из 1962, своп не задействован, ни одного OOM-kill и ни одного перезапуска контейнера.
...
Кэш компиляции после сборки остаётся на диске. На сервере с небольшим диском это заметно — освободить его можно сразу после успешного запуска:

```bash
docker builder prune -f
```

Команда удаляет только кэш сборки: образы, контейнеры и данные не затрагиваются. Следующая пересборка займёт столько же времени, сколько первая, потому что начнёт с нуля.
...
2. Поднимутся три контейнера:
   - **`gotcha`** (собран на Go 1.26) — само приложение: HTTP-сервер, веб-интерфейс, приём событий от SDK, миграции схемы БД при старте.
...
**`gotcha` падает с `exec /usr/local/bin/gotcha: operation not permitted`, а `postgres` и `clickhouse` при этом `Up`.**
Сообщение печатает не приложение, а Docker — процесс не успел стартовать. Образ здесь ни при чём: дело в том, как хост принимает ограничения контейнера. Сервис `gotcha` отличается от баз тем, что работает от непривилегированного пользователя (uid 10001), без единой capability, с файловой системой только на чтение, с `no-new-privileges` и лимитом на число процессов. Какое-то из этих ограничений хост не пропускает — какое именно, показывают две пробы.

**1. Все ограничения разом, но на постороннем образе.**
```bash
...
```
Если вместо `ok` пришёл тот же отказ — виноват набор ограничений, а не Gotcha. Убирайте их из команды по одному, пока проба не пройдёт: снятое последним и есть причина. Начать удобнее с `no-new-privileges` — он отключается через `.env`, без правки compose-файла:
```env
GOTCHA_COMPOSE_NO_NEW_PRIVS=false
```
Это ослабляет один слой защиты из нескольких: контейнер по-прежнему работает от непривилегированного пользователя, без единой capability и с файловой системой только на чтение.

**2. Наш образ со всеми ограничениями, но вместо приложения — шелл.**
```bash
docker compose run --rm --no-deps --entrypoint /bin/sh gotcha \
  -c 'ls -l /usr/local/bin/gotcha; /usr/local/bin/gotcha --version'
```
Имеет смысл, если первая проба напечатала `ok`. Отказ на самом `/bin/sh` означает, что дело в образе — пересоберите его начисто (`docker compose build --no-cache gotcha`). Если шелл запустился, а `--version` не выполнился, дело в самом бинарнике, и `ls -l` в том же выводе покажет его права и размер.

Что бы ни показали пробы, полезно посмотреть на хост:
```bash
docker info --format '{{.SecurityOptions}}'
sudo dmesg | grep -iE 'apparmor|audit' | tail -10
```

Если обе пробы прошли (`ok` в первой, версия приложения во второй), а отказ наблюдался до них — дефект был в самом образе: пересборка начисто устраняет его.

```bash
docker compose build --no-cache gotcha
```

Перед пересборкой проверьте свободное место (`df -h`): сборке нужен запас сверх рабочего набора, см. «Системные требования».
...
ClickHouse не прошёл проверку здоровья, и приложение из-за этого не стартовало. Начните не с логов, а с ресурсов:
...
Обычная причина — нехватка памяти. Минимум для Gotcha — 2 ГБ, и при меньшем объёме ClickHouse не поднимается, причём в его логах может не остаться ничего внятного: процесс снимает ядро по нехватке памяти. На сервере ровно с 2 ГБ запускайтесь с оверлеем для слабого железа — см. «Если сервер на минимуме». Если памяти достаточно, причина отказа будет в `docker compose logs clickhouse`.
