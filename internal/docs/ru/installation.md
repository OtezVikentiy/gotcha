...
Образ здесь ни при чём: это сообщение печатает не приложение, а Docker — ещё до того, как процесс успел стартовать. Так выглядит отказ AppArmor на хосте, где сам `dockerd` работает под профилем безопасности, то есть **Docker установлен из snap**. Ядро запрещает такому `dockerd` переводить контейнер в другой профиль, когда у контейнера выставлен `no-new-privileges`, а этот флаг во всём стеке есть только у сервиса `gotcha` — потому базы и поднимаются нормально.

Проверяется одной командой, без участия Gotcha:
```bash
docker run --rm --security-opt no-new-privileges:true alpine:3.21 echo ok
snap list docker      # показывает, установлен ли Docker из snap
```
Если вместо `ok` напечаталось `exec /bin/echo: operation not permitted` — причина именно эта.

Лечение — переставить Docker из официального источника вместо snap. Учтите: **`snap remove docker` удаляет вместе с собой все образы, контейнеры и тома, созданные snap-докером**, — если в них уже есть данные, сначала сохраните их.
```bash
sudo snap remove docker
curl -fsSL https://get.docker.com | sudo sh
sudo usermod -aG docker $USER     # и перелогиниться
```
Если менять установку Docker нельзя, снимите флаг в `.env` и поднимите стек заново:
```env
