...
- **Срок хранения распространяется и на сводные записи.** Группы ошибок и групп производительности (заголовок, culprit) удаляются, когда последнее их событие старше `GOTCHA_RETENTION_DAYS`; закрытые инциденты аптайма, регрессии производительности и профилей, инциденты по метрикам — когда старше этого срока их закрытие. Заголовок группы — свободный текст из вашего приложения, и переживать объявленный срок хранения он не должен. Открытые инциденты и регрессии не удаляются: они описывают то, что происходит сейчас. При `GOTCHA_RETENTION_DAYS=0` не удаляется ничего.
...
|---|---|---|
| Telegram | ссылка оповещения и `chat_id` получателя; текст — только при `GOTCHA_EXTERNAL_CHANNEL_DETAILS=true` | серверы за пределами РФ |
...
| Telegram | никогда: получатель задан `chat_id`, домена у него нет, а сервис по определению вне вашего контура |
...
`GOTCHA_EXTERNAL_CHANNEL_DETAILS=true` снимает ограничение целиком — детали пойдут любому получателю, включая Telegram. Включайте это, только если у вас есть законные основания для трансграничной передачи. (Переопределение адреса Telegram API настройкой не выведено: оно существует только полем в коде.)
