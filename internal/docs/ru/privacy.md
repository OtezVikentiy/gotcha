...
- **Сроки хранения (retention).** TTL применяется принудительно и настраивается: события и др. — `GOTCHA_RETENTION_DAYS`, спаны, метрики, профили, логи (`GOTCHA_LOG_RETENTION_DAYS`) — свои параметры (см. [Конфигурация](/docs/configuration)). Данные автоматически удаляются из ClickHouse по истечении срока (нулевой срок отключает удаление в своём классе).
