...
### Секреты, записанные на dev-ключе: ротация их не возвращает

Если инстанс какое-то время работал на встроенном dev-ключе (обычный путь:
обкатка, потом «настроим по-настоящему»), описанная выше процедура здесь не
применяется вовсе — `GOTCHA_SECRET_KEY_PREV`, равный dev-ключу, приложение
отклонит при старте: шифровать было нечем, а значит и ротировать нечего. Но
это не значит, что всё в порядке. Каждый секрет, записанный или изменённый
за это время, ушёл в базу открытым текстом и уже прошёл через WAL PostgreSQL,
попав в любой снапшот, `pg_dump` или реплику, снятые в этот период. Установка
настоящего ключа зашифрует то, что лежит в базе СЕЙЧАС, — уже сделанные
бэкапы и архив WAL она не трогает и не может, сколько ключей вы после этого
ни ставьте.

Единственное лечение здесь — не перешифровка, а отзыв и перевыпуск секрета у
источника. Если инстанс держался на dev-ключе какое-то время, отозвать и
перевыпустить нужно:

- bot-токен Telegram у каждого канала оповещений с `kind = "telegram"`
  (перевыпуск — `/revoke` у `@BotFather`);
- секрет HMAC-подписи вебхука у каждого канала с `kind = "webhook"`;
- `client_secret` у каждого OIDC/Yandex ID/VK ID приложения, подключённого
  как SSO (перевыпуск — в консоли провайдера);
- значения HTTP-заголовков (`Authorization` и любые другие секретные
  заголовки) у HTTP-мониторов аптайма — перевыпуск у стороны, выдавшей
  токен или ключ.

Список короткий для типичного инстанса (канал-два оповещений, один
SSO-провайдер), а стоимость пропущенного пункта — учётные данные, читаемые
из следующего же бэкапа.
