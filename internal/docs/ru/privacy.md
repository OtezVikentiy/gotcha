...
**Прежде чем начинать: шаг 1 разлогинивает всех пользователей.** Ключ подписи
сессионных и OAuth-cookie выводится из того же мастер-ключа, что и шифрование
at-rest, поэтому смена `GOTCHA_SECRET_KEY` обнуляет действующие сессии. Это
неизбежная часть ротации в текущем дизайне, а не поломка — предупредите
пользователей заранее, а не разбирайтесь с жалобами постфактум.
...
1. Задайте `GOTCHA_SECRET_KEY_PREV=<старый ключ>` и новый
   `GOTCHA_SECRET_KEY=<новый ключ>`, перезапустите инстанс. На старте всё
   читаемое (client secret SSO, токены каналов, заголовки мониторов)
   перешифровывается новым ключом; итог по каждому из трёх хранилищ пишется
   в лог, вместе с id ключей кольца.
2. Убедитесь, что в итоговых строках лога ноль пропущенных, и проверьте
   прямым запросом к БД, что конвертов со старым id не осталось (`<old-id>` —
   из лога на шаге 1):
