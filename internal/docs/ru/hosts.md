...
Готовый YAML с подставленными `BaseURL` инстанса и активным публичным ключом проекта показывается прямо в интерфейсе — в пустом состоянии списка хостов и на странице настроек порогов (кнопка «Показать конфиг коллектора»), с кнопкой копирования в буфер. По форме это тот же конфиг, что и здесь:
...
`system.cpu.logical.count` включена явно: без неё не с чем делить load average «на ядро» ни для графика, ни для порога нагрузки. Остальные метрики каждого скрейпера включены дефолтным набором версии `otelcol-contrib` — явно перечислены только те, что нужны графикам и порогам напрямую.
...
Готовый конфиг из шага 2 включает всё необходимое для полного набора графиков и порогов из коробки.
...
## Свой агент — в будущих версиях

Сейчас единственный поддерживаемый способ собрать хостовые метрики — сторонний `otelcol-contrib`. В будущих версиях появится собственный лёгкий агент Gotcha как более простая альтернатива для установки — контракт этой страницы (имена метрик, промоушен `host.name`) уже рассчитан на совместимость с ним: агент будет слать те же имена метрик тем же протоколом.
