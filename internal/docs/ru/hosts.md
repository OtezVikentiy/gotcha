...
Инциденты (открытие/эскалация/закрытие) оценивает фоновый процесс с интервалом `GOTCHA_HOST_EVAL_INTERVAL` (по умолчанию 60 секунд, минимум 1 секунда, см. [Конфигурацию](/docs/configuration)); уведомления уходят в каналы проекта тем же общим механизмом, что у [Оповещений](/docs/alerts), а дальнейшее продвижение по ступеням настраивается на странице [«Эскалации»](/docs/escalations).
