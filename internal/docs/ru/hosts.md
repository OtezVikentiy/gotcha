...
- скачивает бинарь агента и сверяет его SHA-256 с `SHA256SUMS` — оба файла раздаёт сам инстанс (`GOTCHA_AGENT_DIST_DIR`, см. [Конфигурацию](/docs/configuration)), внешние сети агенту для установки не нужны — это работает и в закрытых периметрах без выхода в интернет; если инстанс собран без Docker-образа (например, локальный `make go-build` без сборки образа), `/install.sh` отвечает `404` — бинарники агента кладёт в образ именно докер-сборка (`GOTCHA_AGENT_DIST_DIR`);
...
Инциденты (открытие/эскалация/закрытие) оценивает фоновый процесс с интервалом `GOTCHA_HOST_EVAL_INTERVAL` (по умолчанию 60 секунд, минимум 1 секунда, см. [Конфигурацию](/docs/configuration)); уведомления уходят в каналы проекта тем же общим механизмом, что у [Оповещений](/docs/alerts).
...
Резолвер разбирает каждый вид порога **независимо** от остальных трёх, и внутри вида **enabled и value резолвятся раздельно**: включённость и число могут взяться с разных уровней каскада — например, хост наследует число с уровня роли, но сам решает, включён ли порог.

На каждом уровне (хост, роль, окружение) у каждого вида — три состояния (tri-state):
...
- **Выключить** — не оценивать порог на этом уровне независимо от того, что задано выше; выключенный порог по-прежнему хранит унаследованное число — если его снова включить, показывать нечего мусорное.
...
- [Конфигурация](/docs/configuration) — переменные окружения инстанса, включая `GOTCHA_HOST_EVAL_INTERVAL` и `GOTCHA_METRIC_RETENTION_DAYS`.
