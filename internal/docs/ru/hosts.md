...
Оба способа ниже — и агент, и коллектор — регистрируют хост, и оба используют
ключ проекта типа `agent`: это единственный тип ключа, которым можно
зарегистрировать хост (подробнее — [Ключи приёма](/docs/keys)). Готовые
команда/конфиг в интерфейсе уже подставляют такой ключ; выпустить его заново
...
Агент собирает тот же набор метрик `system.*`, что и поставляемый конфиг коллектора (см. таблицу ниже), плюс `system.uptime`, тем же протоколом OTLP — контракт страницы (имена метрик, промоушен `host.name`) для агента и коллектора один и тот же, поэтому переключение между ними не требует ничего менять на стороне Gotcha. Экспорт по умолчанию раз в 30 секунд. Если инстанс временно недоступен (перезапуск, сеть), агент буферизует недоставленные батчи в памяти — при дефолтном интервале это около **часа** истории (120 батчей **или 8 МиБ — что наступит раньше**), — и досылает их при восстановлении связи, ничего не теряя на короткий сбой. Сбой дольше этого часа буфер всё же переполняет, и самые старые метрики за это время безвозвратно теряются; агент называет эту потерю честно, а не молчит о ней — она видна в его собственном журнале и едет на инстанс отдельной метрикой `gotcha.agent.metrics_dropped` (не входит в таблицу `system.*` ниже — это самонаблюдение агента, а не показание хоста): кумулятивный счётчик потерянных точек с момента запуска процесса, растёт только при потере. По этой метрике можно завести обычное правило алерта — рост означает дыру в данных за период отказа, и значения за неё не вернутся, даже когда связь восстановится.
...
- `401` в журнале — ключ в `/etc/gotcha-agent/gotcha-agent.env` не тот (например, ключ отозвали): поправьте `GOTCHA_AGENT_INGEST_KEY` и выполните `sudo systemctl restart gotcha-agent`;
- ошибок в журнале нет, агент шлёт без сбоев, а точки метрик хоста видны на общей странице «Метрики» — ключ рабочий, но не того типа: в `GOTCHA_AGENT_INGEST_KEY` попал ключ из DSN проекта (`browser`/`server`) вместо ключа типа `agent`. Приём такой ключ принимает как обычные метрики, но регистрацию хоста им не разрешает — перевыпустите или скопируйте заново ключ типа `agent` в настройках проекта (см. «Подключение» выше);
...
|---|---|---|---|
| `GOTCHA_AGENT_ENDPOINT` | да | — | Базовый URL инстанса, без пути (тот же смысл, что `endpoint` в конфиге коллектора). Должен быть абсолютным `http(s)`-адресом без запроса и фрагмента; хвостовая косая срезается сама. |
| `GOTCHA_AGENT_INGEST_KEY` | да | — | Ключ проекта типа `agent` (см. «Подключение» выше) — **не** публичный ключ из DSN проекта: тот несёт тип `browser`/`server`, приём с ним ответит `200`, но хост не зарегистрируется никогда (см. [Ключи приёма](/docs/keys)). Тот же ключ идёт и в заголовок `Authorization` конфига коллектора ниже. |
| `GOTCHA_AGENT_INTERVAL_SECONDS` | нет | `30` | Интервал сбора и отправки метрик, целое число секунд. Допустимый диапазон **10–300**: меньше — риск самоDoS ключом по приёму, больше — ложные срабатывания порога «Тишина». |
...
| `GOTCHA_AGENT_TLS_INSECURE_SKIP_VERIFY` | нет | `false` | Полностью отключает проверку TLS-сертификата инстанса (`1`/`0`/`true`/`false`/`yes`/`no`/`on`/`off`, без учёта регистра). Крайнее средство — предпочитайте `GOTCHA_AGENT_CA_CERT`, это отключает защиту от MITM на канале доставки метрик. |
...
**Имя хоста — ключ идентичности, а не подпись.** Хост в проекте определяется парой «проект + `host.name`»: приём ищет запись по имени и обновляет её, а не найдя — заводит новую. Поэтому смена `GOTCHA_AGENT_HOSTNAME` (или системного hostname, если переопределения нет) — это **не переименование**: под новым именем появится **новый** хост с пустой историей, а старый останется в списке со своими метками, порогами-переопределениями и рёбрами [подавления шторма](/docs/alert-suppression), будет молчать и в конце концов откроет инцидент «Тишина», а после срока хранения метрик его снимет джанитор (см. [«Удаление и автоматическая очистка»](#udalenie-i-avtomaticheskaya-ochistka)). Переименования хоста на лету в интерфейсе нет. Если имя менять всё же нужно: перенесите переопределения порогов и рёбра зависимостей на новый хост руками и удалите старый с карточки — иначе он будет пейджить о тишине до самой автоочистки.

Как и сервер (см. [Конфигурацию](/docs/configuration)), агент отказывает старту на опечатке ВНУТРИ своего же неймспейса — переменной с префиксом `GOTCHA_AGENT_`, которой нет в таблице выше (`GOTCHA_AGENT_INTERVAL_SECOND` без `S` на конце и подобные). Чужую `GOTCHA_*` в общем `.env` хоста (например, серверную `GOTCHA_PG_DSN`, если сервер и агент стоят рядом) агент по-прежнему игнорирует — он никогда её не читает, отказ по ней был бы самоуправством.
...
`endpoint` — это **базовый** URL инстанса, без `/v1/metrics`: путь дописывает сам экспортёр `otlphttp`. Ключ в заголовке `Authorization` — ключ проекта типа `agent` (см. «Подключение» выше и [Ключи приёма](/docs/keys)), **не** ключ из DSN проекта: тот несёт тип `browser`/`server` и с ним хост не зарегистрируется — приём ответит `200`, точки лягут в общие «Метрики», но в `/projects/{id}/hosts` хост не появится никогда, без единой ошибки в журнале коллектора. Граница видимости конфига та же, что у DSN — доступен всем с доступом к проекту.
...
С шагом сбора 30 секунд и сетевой доставкой первая точка обычно долетает до приёма в пределах минуты после старта коллектора — откройте `/projects/{id}/hosts` и обновите страницу. Если хост не появляется дольше пары минут: `systemctl status otelcol-contrib` и `journalctl -u otelcol-contrib` покажут, доходит ли экспорт вообще (неверный или отозванный ключ → приём отвечает `401`, экспортёр логирует это в журнал коллектора). Если журнал коллектора чист, а точки хоста тем не менее видны на общей странице «Метрики», но не на «Хостах» — ключ в заголовке `Authorization` рабочий, но не типа `agent`: подставлен ключ из DSN проекта (`browser`/`server`), который метрики принимает, а хост регистрировать им нельзя (см. «Подключение» выше).
...
- Переименования хоста нет: `host.name` — ключ идентичности, новое имя заводит новый хост (см. [«Переменные окружения агента»](#peremennye-okruzheniya-agenta)).
