...
- Порог/статус per-host не переопределяются — настройка на странице `/hosts/settings` действует на все хосты проекта разом. Отдельные пороги на хост, метки и авторегистрация со статусами — не в этой версии.
