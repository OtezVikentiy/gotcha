...
| `GOTCHA_AGENT_TLS_SKIP_VERIFY` | нет | `false` | Полностью отключает проверку TLS-сертификата инстанса (`1`/`0`/`true`/`false`/`yes`/`no`/`on`/`off`, без учёта регистра). Крайнее средство — предпочитайте `GOTCHA_AGENT_CA_CERT`, это отключает защиту от MITM на канале доставки метрик. |
