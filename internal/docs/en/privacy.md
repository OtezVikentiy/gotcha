...
- **Retention covers summary records too.** Error and performance issues (title, culprit) are deleted once their last event is older than `GOTCHA_RETENTION_DAYS`; closed uptime incidents, performance and profile regressions, and metric incidents are deleted once they were closed longer ago than that. An issue title is free text from your application, and it must not outlive the retention you declared. Open incidents and regressions are never deleted — they describe what is happening right now. With `GOTCHA_RETENTION_DAYS=0` nothing is deleted.
