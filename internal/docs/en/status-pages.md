...
No. The `/status/{slug}` page opens without logging in — link to it from anywhere: for customers, partners, or a support channel. It carries no dashboard navigation.
...
Yes. Pages are managed as a list on `/projects/{id}/statuspages`, each with its own slug and its own set of monitors — e.g. one page for customers and a more detailed one for partners.
