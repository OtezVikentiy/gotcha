...
| `GOTCHA_COMPOSE_BIND` | `127.0.0.1` | Host address the app port is published on. Loopback only by default — the port isn't reachable from outside the server until you explicitly set `0.0.0.0` (see [Installation](/docs/installation)). The app itself never sees this variable and cannot tell whether its port is published externally — keep `/metrics`, `/version`, `/healthz`, `/readyz` behind a reverse proxy or firewall, see [Hardening](/docs/hardening). |
...
default, with your own reverse proxy publishing the port — see [Installation without
...
| Variable | Default | Description |
|---|---|---|
| `GOTCHA_EDITION` | `oss` | `oss` or `saas`, case-insensitive (`OSS`/`SaaS` are accepted too). Controls the **default** for the quota variables below: in `oss` all defaults are `0` (unlimited); in `saas`, `1,000,000`/month. An explicit `GOTCHA_DEFAULT_*_QUOTA` always overrides the edition default. |
...
| `GOTCHA_INGEST_RATE_PER_SEC` | `500` | Per-DSN ingest rate limit: requests per second per project (token bucket, burst = 2× the limit). Checked after DSN authentication and before quotas; over the limit the API replies `429` with a short `Retry-After`. `0` disables the limit. The bucket lives in the ingest process's memory, separately on each `--mode=ingest`/`all` replica — behind a load balancer with N such replicas, the effective ceiling per project reaches N× the variable's value, not the value itself. |
| `GOTCHA_INGEST_PREAUTH_RATE_PER_SEC` | `2000` | Rate limit by client IP, checked before DSN authentication on every ingest route (token bucket, burst = 2× the limit). Higher than `GOTCHA_INGEST_RATE_PER_SEC` on purpose — one IP legitimately serves several DSNs (a shared app server for multiple projects). `0` disables the limit. |
| `GOTCHA_INGEST_SIGNAL_RATE_PER_SEC` | `2` | Rate limit by client IP for recording an invalid-key signal (token bucket, burst = 5× the limit) — much tighter than `GOTCHA_INGEST_PREAUTH_RATE_PER_SEC`: a legitimate mismatched DSN keeps hitting its own project repeatedly, not enumerating others, so infrequent touches shouldn't hit the limit. `0` disables the limit. |
...
| `GOTCHA_SCRUB_DENY_KEYS` | built-in list (`password,passwd,pwd,pass,token,secret,authorization,auth,cookie,api_key,apikey,access_token,refresh_token,session,credit_card,card_number,cvv`) | Comma-separated, case-insensitive key names whose values get redacted in tags/contexts/stack traces/span data. This variable **extends** the built-in list rather than replacing it, so adding your own key (e.g. `internal_user_id`) takes one value and does not require re-listing the standard ones. To drop a specific built-in key, name it exactly in `GOTCHA_SCRUB_KEEP_KEYS`. |
| `GOTCHA_SCRUB_KEEP_KEYS` | empty | Comma-separated exact names to exempt from the denylist. Matching is deliberately fail-closed — a name is redacted when it *contains* a denylist word, so `author` (contains `auth`) and `tokenizer` (contains `token`) are redacted by default. Under-redacting leaks personal data; over-redacting only costs a debugging field, and this setting brings it back: `GOTCHA_SCRUB_KEEP_KEYS=author,tokenizer`. |
...
| `GOTCHA_ESCALATION_INTERVAL_SECONDS` | `60` | How often (seconds) the escalation scheduler checks open, unacknowledged incidents and advances their [ladder](/docs/escalations) to the next step. Minimum 1 second. Gated by the same `GOTCHA_EVALUATORS_ENABLED` as the five cycles above. |
...
| Variable | Default | Description |
|---|---|---|
| `GOTCHA_LOGGING_LEVEL` | `info` | Log verbosity: `debug`, `info`, `warn` (alias `warning`) or `error`, case-insensitive. Raise it to `debug` to get more detail during an incident without rebuilding. An unrecognized value refuses startup rather than silently falling back to `info`: a typo in the log configuration (e.g. `trace`) needs to be caught right away, not discovered mid-incident when the extra detail turns out to be missing. |
| `GOTCHA_LOGGING_FORMAT` | `text` | Log format: `text` or `json`, case-insensitive. Use `json` when shipping logs to Loki/ELK. An unrecognized value refuses startup. |

## Security

| Variable | Default | Description |
|---|---|---|
| `GOTCHA_SECRET_KEY` | `insecure-dev-secret` | Signs session and OAuth state cookies, and doubles as the master key for at-rest encryption (SSO client secrets, channel tokens, monitor headers). **The default is public** (it's in the source code) — leaving it on a real server allows account takeover via OAuth login. On a non-localhost `GOTCHA_BASE_URL`, the app refuses to start in the `web`, `all`, `ingest`, and `uptime` modes (everywhere except `probe`) until a real key is set, and requires it to be **at least 32 bytes** (a shorter key is a weak signature). Generate one with `openssl rand -hex 32`. Both checks can be waived for an unusual dev setup with `GOTCHA_SECRET_KEY_ALLOW_INSECURE=1`. The value is trimmed — `"key "` and `"key"` produce the same key, so a stray space from copy-pasting doesn't change the encryption; a whitespace-only value refuses to start instead of silently falling back to the public default. See [Installation](/docs/installation), step 5, for the full walkthrough. Change the key on a running instance only via the rotation procedure — see [Privacy and 152-FZ](/docs/privacy). |
| `GOTCHA_SECRET_KEY_PREV` | *(empty)* | The previous master key, set for the duration of a `GOTCHA_SECRET_KEY` rotation — set alongside the new `GOTCHA_SECRET_KEY` during the transition, then removed. While set, the app decrypts with either key but encrypts only with the current one. **Unlike `GOTCHA_SECRET_KEY`, the value is read verbatim, with no trimming of surrounding whitespace** — so rotation still works for a key that was stored with a trailing space (trimming it would turn it into a different key and block decryption). No trimming doesn't mean anything goes: a whitespace-only value is the same hygiene error as `GOTCHA_SECRET_KEY`/`GOTCHA_PG_DSN`/`GOTCHA_CH_DSN` (most likely a stray space from copy-pasting, not a deliberate "key made of spaces"), and refuses to start regardless of mode rather than silently becoming a "real" previous key. The app refuses to start with a nonsensical key pair: `GOTCHA_SECRET_KEY_PREV` equal to the current `GOTCHA_SECRET_KEY`, or either one equal to the default dev key while `GOTCHA_SECRET_KEY_PREV` is set. `GOTCHA_SECRET_KEY_ALLOW_INSECURE=1` does not waive these checks — they're not about key strength, they're about a config that physically can't do what's expected of it. See [Privacy and 152-FZ](/docs/privacy) for the rotation procedure. |
| `GOTCHA_TRUSTED_PROXIES` | *(empty)* | Comma-separated CIDRs (`10.0.0.0/8`) and/or bare IPs (`192.168.1.5`, treated as `/32` or `/128`) of reverse proxies you trust. Behind a proxy (the recommended [install](/docs/installation) topology), set this to the proxy's address so the per-IP login rate limiter keys on the real client IP from `X-Forwarded-For` instead of the proxy's — otherwise every login attempt looks like it comes from the proxy and the limiter can't tell clients apart. Invalid entries are a startup error, not a silent skip. The bare-metal installer sets this to `127.0.0.1/32,::1/128` — a proxy on the same host. |
| `GOTCHA_SECRET_KEY_ALLOW_INSECURE` | `false` | Escape hatch that bypasses the check above — lets the app start with the default key even on a non-localhost address. **Development-only**; never set this on a real deployment. The value is parsed on every start unconditionally, regardless of whether `GOTCHA_SECRET_KEY` is already strong enough: an unrecognized value (not `1`/`0`/`true`/`false`/`yes`/`no`/`on`/`off`) refuses to start instead of silently letting the typo through. |
| `GOTCHA_REGISTRATION_MODE` | `invite` | Self-registration mode (case-insensitive): `open` — anyone can register; `invite` — an account is created only against a valid invitation (by password or through a provider); `closed` — **no new accounts appear at all, not even with a valid invitation**. That is the difference: under `closed` an invited person hits "registration is closed", and inviting anyone from outside becomes impossible — invitations only work for people who already have an account. The very first user on a fresh instance can always register regardless of this setting (instance-admin bootstrap). |
...
| `GOTCHA_AUTO_MIGRATE_ENABLED` | `true` | Apply database schema migrations automatically on startup. `false` means migrations must be applied as a separate step beforehand — otherwise the app refuses to start against a schema that's out of date. See [Upgrade](/docs/upgrade) for details and when this is needed. |
| `GOTCHA_EXTERNAL_CHANNEL_DETAILS_ENABLED` | `false` | Whether to send error text (title/culprit/body) to external alert channels (Telegram/webhook). `false` sends only an anonymized link back to the instance, without the error text (which may contain personal data you don't want leaving the instance). |
...
`web` and `ingest` are safe to scale to multiple replicas: they don't coordinate with each other, but they don't duplicate each other's work either. Inside `uptime`/`all`, the check scheduler and runner are also safe across replicas — queuing a check is idempotent (`internal/uptime/queue.go`: `FOR UPDATE OF m SKIP LOCKED` plus `ON CONFLICT (monitor_id, region) DO NOTHING`), and execution fairly splits the queue between replicas (`FOR UPDATE OF q SKIP LOCKED`); the watchdog is the same story — missed heartbeats, SSL alerts and reminders are all claimed atomically (`internal/uptime/watchdog.go`, the `Claim*` methods) — so an extra replica adds throughput, not duplicate pages. **The six evaluators are what still require exactly one replica** (performance regressions, metric rules, profile regressions, host thresholds, SLO burn rate, the escalation scheduler — see [Limits and evaluators](#limits-and-evaluators)): each ticks independently on every replica with no coordination between them. **That is why `uptime` and `all` must each still run as exactly one replica** — a second replica doesn't add a standby, it runs two parallel passes over the same rules and thresholds on every tick.
...
| `GOTCHA_UPTIME_LOCAL_REGION` | `local` | The name of the built-in local uptime-check region — what's shown in the UI when picking a monitor's region. |
| `GOTCHA_PROBE_KEY` | *(empty)* | `--mode=probe` only: the bearer token this probe authenticates to the central instance with. Required in this mode. |
...
| `GOTCHA_OIDC_TRUST_EMAIL` | `false` | Whether to trust this OIDC provider's claimed `email`/`email_verified` enough to auto-provision a new account from it (open registration or an invite) or auto-link a login to an existing account by matching email. Defaults to `false`: without this variable, OIDC login only works for accounts already linked (see [SSO](/docs/sso)). Turn it on only for your own single-tenant IdP (a Keycloak/Authentik/Zitadel, etc. that you control). **Do not enable it** for a public multi-tenant IdP (a shared Google or general-purpose Auth0 tenant) where anyone can sign up and claim any email as their own — that opens the door to hijacking a password account by its email address. |
