...
| **MODX Revolution** 3 | [gotcha (transport)](https://getgotcha.ru/downloads/gotcha-1.0.0-pl.transport.zip) | PHP errors (handler chain), transactions by template (`web:BaseTemplate`), JS errors and Web Vitals |
...
| **Per-DSN rate limit hit** | The second source of `429`: each project is capped at `GOTCHA_INGEST_RATE_LIMIT` requests per second (500 by default, burst 2×). Unlike the quota, the `Retry-After` here is about a second — SDKs with built-in backoff recover on their own. A sustained hit usually means an event loop is misfiring (an error per request, a retry storm). |
