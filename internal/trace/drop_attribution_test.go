...
// Спан без OrgID (0) не атрибутируется в сток; спановый сток отдельный от
// SetDropSink — дроп спана не квота, задваивать dropped_transactions нельзя.
func TestSpanWriterAttributesSpanDropsPerOrg(t *testing.T) {
	var mu sync.Mutex
	got := map[int64]int64{}
	w := NewSpanWriter(&fakeCHConn{})
	w.maxSpanBuf = 3       // маленький потолок spanBuf
	w.interval = time.Hour // не флашим по тику
	w.batchSize = 100      // txBuf не флашим по наполнению
	w.spanBatchSize = 100  // spanBuf тоже
	w.SetSpanDropSink(func(orgID, n int64) {
		mu.Lock()
		got[orgID] += n
		mu.Unlock()
	})

	// Каждый Add кладёт ровно один спан (root, без детей): oldest→newest —
	// org1, org0 (без атрибуции), org2.
	for _, org := range []int64{1, 0, 2} {
		w.Add(org, 1, sampleTx(0))
	}
	// Три новых спана org3 выбивают три самых старых: org1, org0 (не
	// атрибутируется), org2.
	for i := 0; i < 3; i++ {
		w.Add(3, 1, sampleTx(0))
	}
