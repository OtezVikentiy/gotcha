...
// копиями в cmd/gotcha/config.go и internal/agent/config.go (ops-review E3
// T8 круг 1). Две функции с явными именами вместо одной с сентинелом `nil`
// — ops-review E3 T8 круг 2 нашёл, что `nil` (весь реестр) и пустой,
// но НЕ-nil срез (в тогдашней сигнатуре — «нет ключей») визуально
// неотличимы на месте вызова, и мутация `nil` → `[]string{}` (обычный
// результат make/фильтрации/decode пустого JSON) молча выключала fail-fast
// целиком, не роняя ни одного теста, ни компилятора, ни go vet. Раздельные
// функции убирают вопрос: передать «ничего не проверять» вместо «весь
// реестр» больше не через один параметр, который делает диаметрально
// разные вещи в зависимости от длины.
//
