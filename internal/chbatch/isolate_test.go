...
// Ничего не дропается — все ряды возвращаются в unresolved в исходном порядке.
...
func TestIsolatePoison_MixedPoisonAndTransient(t *testing.T) {
	serverBad := map[int]bool{2: true}
	transient := map[int]bool{5: true}
	insert := func(_ context.Context, rows []int) error {
		for _, r := range rows {
			if serverBad[r] {
...
func TestIsServerDataError(t *testing.T) {
	cases := []struct {
		...
		// Транзиентные серверные исключения — не яд: иначе перегрузка CH дропала бы валидные батчи.
