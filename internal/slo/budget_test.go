...
func TestBudgetFractions(t *testing.T) {
	...
	over := []slo.Bucket{{Good: 980, Total: 1000}}
	...
	good := []slo.Bucket{{Good: 999, Total: 1000}}
	...
	c, ok := slo.BudgetConsumedFraction(tight, 0.999)
...
func TestBurnRate(t *testing.T) {
	...
	cool := []slo.Bucket{{Good: 999, Total: 1000}}
...
func TestDecideBurn(t *testing.T) {
	...
	hot := []slo.Bucket{{Good: 800, Total: 1000}}
	cool := []slo.Bucket{{Good: 1000, Total: 1000}}
	...
	// порог берём из самой формулы, чтобы избежать FP-неточности равенства при сравнении границы.
