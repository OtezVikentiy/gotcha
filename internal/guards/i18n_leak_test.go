...
var legitExemptions = []Exemption{
	...
	{Value: `"требуется ручной force перед стартом", label, got)`, Why: "продолжение того же сообщения (see выше)", Finding: "по замыслу"},
	...
	{Value: `"предыдущая миграция оборвалась; проверьте схему и выполните ручной force "+`, Why: "продолжение сообщения об оборвавшейся миграции ниже", Finding: "по замыслу"},
	{Value: `"(migrate force %d) перед перезапуском: %w", dir, derr.Version, derr.Version, err)`, Why: "продолжение сообщения об оборвавшейся миграции — читает оператор в консоли", Finding: "по замыслу"},
...
// maxLegitExemptions — потолок списка исключений «по замыслу»: 27 записей
// ушли на многострочные сообщения проверки схемы БД в internal/db (21
// строка), одну таблицу транслитерации в internal/docs/anchors.go (5 строк)
// и одну ошибку инструментария самого пакета guards (1 строка). Список
// постоянный, а не долг — расти он должен только если появится ещё один
// законный по замыслу случай, и осознанной правкой числа, а не сам по себе.
const maxLegitExemptions = 27
