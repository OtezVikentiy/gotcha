...
// Префикс ключа обязан содержать точку: без этого условия любой короткий литерал
// слева от "+" ("total" + x) становится префиксом и амнистирует всё, что с него
// начинается, — сторож слепнет, а храповик амнистии запирает слепоту как норму.
func isKeyPrefix(p string) bool { return strings.Contains(p, ".") }
...
const (
	...
	minDynamicPrefixes       = 36
	maxPrefixAmnestiedKeys   = 245
...
var orphanKeyExemptions = []Exemption{
	{
		...
		Why:     "каноническая фикстура плюрализации: на формах этого ключа стоят plural_test.go и plural_extra_test.go, включая фоллбэк локали; исключение снимется само, когда ключ подключат в интерфейс",
	},
}
...
func TestCatalogKeysAreReferenced(t *testing.T) {
	...
	if amnestied > maxPrefixAmnestiedKeys {
		t.Fatalf("префиксами амнистировано %d ключей при потолке %d — слишком широкий префикс делает сторожа слепым, сузьте место сборки ключа",
