...
// Префикс ключа обязан начинаться непустым сегментом до точки: без этого условия
// любой короткий литерал слева от "+" ("total" + x) становится префиксом и
// амнистирует всё, что с него начинается, — сторож слепнет, а храповик амнистии
// запирает слепоту как норму. Пустой сегмент (".inner_id." из конкатенации вида
// "." + uuid) не привязан ни к одному неймспейсу ключей и точно так же слеп.
...
const (
	...
	// Порог ограничивает рост неаудируемой зоны (ключи, живые только по счастливому
	// совпадению с чужим префиксом), а не спасает от широкого префикса, заведённого
	// поверх уже используемых ключей, — те отдаются более ранним категориям раньше,
	// чем доходят до амнистии.
