...
//
// Честная граница (проверка синтаксическая, по именам идентификаторов, БЕЗ
// разрешения типов go/types — см. докблок isHTTPErrorCall про алиас/тень
// пакета http, здесь то же самое про i18n, симметрично):
//
//   - (а) локальная переменная по имени i18n со своим методом T (реальный
//     пакет i18n при этом затенён) пройдёт мимо сторожа — isI18nTCall видит
//     ровно селектор с идентификатором "i18n" и именем "T", ему всё равно,
//     ссылается ли "i18n" на пакет или на что-то, объявленное человеком
//     специально, чтобы обмануть проверку;
//   - (б) http.Error, вызванный не напрямую, а через переменную-функцию
//     (`errFn := http.Error; errFn(w, X, status)`), не опознаётся вовсе —
//     isHTTPErrorCall ищет селектор `http.Error` буквально в самом
//     CallExpr.Fun, а не то, куда в итоге ведёт errFn;
//   - (в) обратная сторона той же монеты — обёртка-хелпер, которая ВНУТРИ
//     себя честно зовёт i18n.T(...) и возвращает готовую строку
//     (`func badRequestMsg(ctx) string { return i18n.T(ctx, "error.…") }`,
//     затем `http.Error(w, badRequestMsg(r.Context()), status)`), будет
//     ЛОЖНО помечена нарушением: вторым аргументом стоит вызов badRequestMsg,
//     а не i18n.T, и isI18nTCall об этом ничего не знает.
//
// Пункты (а)/(б) требуют осознанной диверсии (переименовать пакет в
// сигнатуре не получится незаметно, локальная тень бросается в глаза при
// чтении диффа) — от намеренного обхода тестовый сторож не защищает никогда,
// это не его задача (см. систематически те же слова в докблоке ContentAnchor
// про то, чего его схема не гарантирует). Гоняться за ними через go/types
// избыточно ценой сложности сторожа. Пункт (в) — другое дело, это ЛОЖНОЕ
// срабатывание на честном коде, и если такая обёртка когда-нибудь
// понадобится: НЕ вносите её файл в machineResponseFiles — это список для
// ответов, которые i18n не нужен вовсе, а не глушилка ложных срабатываний,
// и один такой прецедент размоет границу для всех следующих файлов в
// списке. Правильных пути два — либо звать i18n.T(...) непосредственно
// аргументом http.Error (как logs.go:274, без промежуточной функции), либо
// расширить isI18nTCall (например, завести явный список разрешённых
// функций-обёрток — тем же приёмом, каким machineResponseFiles ведёт список
// файлов, а не эвристикой).
//
...
//
// Так же наивно, как isHTTPErrorCall — без go/types, по имени идентификатора
// "i18n": локальная переменная с этим же именем и методом T затенит пакет
// и пройдёт проверку (см. докблок TestNoLiteralHTTPErrorInWeb, пункт «а»)
// — сознательная диверсия, а не то, от чего этот сторож защищает.
