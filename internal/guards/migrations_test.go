...
// qa P2-2 (2026-08-12): destructiveForms/destructiveSQL — это статический
// грep по ТЕКСТУ SQL миграции на разрушительные формы (DROP/RENAME/…), а НЕ
// поведенческий тест «старый бинарь переживает применённую миграцию» —
// последнее означало бы реально поднять старую версию кода против уже
// смигрированной новой схемы и погонять её запросы. Для схемы такого размера
// (десятки миграций, продолжает расти) полный поведенческий тест на каждую
// пару версий был бы дорогим и медленным; текстовый грep — сознательно более
// дешёвая граница метода: он ловит ИЗВЕСТНЫЕ разрушительные SQL-формы по
// синтаксису, но не гарантирует совместимость на семантическом уровне
// (например, миграция может быть текстуально безобидной, но поменять смысл
// колонки так, что старый бинарь начнёт молча писать не то). Зафиксировано
// здесь явно как граница метода, а не как гарантия полной обратной
// совместимости — комплементарно TestMigratePGUpDownUp/TestMigrateCHUpDownUp
// в internal/db/migrate_test.go, которые проверяют, что миграции ПРИМЕНЯЮТСЯ
// и ОТКАТЫВАЮТСЯ (up→down→up на всей схеме целиком), а не то, что старый код
// совместим с промежуточным состоянием.
