...
func pageComponents() map[string]templ.Component {
	...
	m := map[string]templ.Component{
		...
		"IssueDetail": IssueDetail(issue.Issue{ID: 5, Title: "NPE", Level: "error", Status: "unresolved", TimesSeen: 3, FirstSeen: now, LastSeen: now, AssigneeID: func() *int64 { v := int64(2); return &v }(), AssigneeEmail: "m@x.io"}, members, stubC, []event.Stored{ev}, "e1", &ev, []Frame{{Function: "f", Filename: "a.go", Lineno: 3, InApp: true}}, "u@e.com"),
		...
		"OrgSettings":     OrgSettings(o, members, 1, []QuotaVM{{Kind: "События", Field: "event_quota", Usage: 50, Limit: 1000}, {Kind: "Транзакции", Field: "transaction_quota", Usage: 0, Limit: 0}}, true, "err", "https://g/invite/t", SSOSettings{IsOwner: true, Configured: true, Issuer: "https://idp", ClientID: "c", Domain: "x.io", DefaultRole: "member", Enforced: true, RedirectURI: "https://g/sso"}, "o@x.io", &QuotaBanner{Text: "лимит", Href: "/x"}),
