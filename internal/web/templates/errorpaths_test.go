...
func pageComponents() map[string]templ.Component {
	...
	m := map[string]templ.Component{
		...
		"IncidentsList":        IncidentsList(7, []IncidentRow{{Incident: uptime.Incident{ID: 1, StartedAt: now.Add(-time.Hour), Cause: "t"}, MonitorName: "web"}, {Incident: uptime.Incident{ID: 2, StartedAt: now.Add(-5 * time.Hour), ResolvedAt: &now, Cause: "5xx"}, MonitorName: "api"}}, 1, 2, "u@e.com"),
