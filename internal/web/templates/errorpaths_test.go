...
func pageComponents() map[string]templ.Component {
	...
	m := map[string]templ.Component{
		...
		"HostSettings":       HostSettings(7, host.DefaultSettings(), "curl https://x/install.sh | sh", "cfg", "", nil, "", HostGroupThresholdsVM{}, "u@e.com", true),
		"HostsList":          HostsList(7, []HostRowVM{{Name: "web-1", StatusKind: "ok", CPU: &cpu04, LastSeen: now}}, false, 100, HostsFilterVM{}, HostsFacets{}, nil, "curl https://x/install.sh | sh", "cfg", "", "u@e.com", false, true),
