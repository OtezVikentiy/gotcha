...
func pageComponents() map[string]templ.Component {
	...
	m := map[string]templ.Component{
		"IssuesList":           IssuesList(7, issueRows, IssuesFilter{Status: "unresolved"}, 1, 2, "u@e.com", []string{"production"}, &QuotaBanner{Text: "лимит", Href: "/x"}, GettingStartedVM{ProjectID: 7, Done: 2, Step2Done: true}),
		"IssueDetail":          IssueDetail(issue.Issue{ID: 5, Title: "NPE", Level: "error", Status: "unresolved", TimesSeen: 3, FirstSeen: now, LastSeen: now, AssigneeID: func() *int64 { v := int64(2); return &v }(), AssigneeEmail: "m@x.io"}, members, stubC, TimeRangeVM{Key: "24h"}, []event.Stored{ev}, "e1", &ev, []Frame{{Function: "f", Filename: "a.go", Lineno: 3, InApp: true}}, "u@e.com", false, false, "", ""),
		...
		"RegressionsList":      RegressionsList(7, []trace.Regression{{ID: 1, TargetKind: "endpoint_p95", Target: "GET /", Metric: "duration", Status: "open", BaselineValue: 100, PeakValue: 300, StartedAt: now}, {ID: 2, TargetKind: "webvital_p75", Target: "/home", Metric: "lcp", Status: "resolved", BaselineValue: 2000, PeakValue: 4000, StartedAt: now.Add(-2 * time.Hour), ResolvedAt: &now}}, []string{"после деплоя v1.2.3 (5 минут назад)", ""}, "open", "u@e.com", false),
