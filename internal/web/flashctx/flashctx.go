...
type Flash struct {
	...
	N int
	// M — второе число для парных сообщений («создано N, пропущено M», B6).
	// Осмысленно только вместе с Pair.
	M int
	// Pair — ключ объявлен парным (белый список flashPairKeys, web/flash.go):
	// шаблон подставляет {n}/{m} через i18n.Tf вместо плюрального Tn. Флаг
	// ставит parseFlash по ключу — сам он через cookie не передаётся и
	// клиентом не подделывается.
	Pair bool
}
