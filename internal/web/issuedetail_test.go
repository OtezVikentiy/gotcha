...
// url.Values.Encode() сортирует параметры по имени (end= раньше start=) — порядок не
// фиксирован, оба паттерна проверяются независимо.
var (
...
// Окно ±5м всегда, безусловно: без него события старше суток отсекались бы дефолтным
// 24ч-окном /logs.
