...
// дедлайном): снятый инцидент обязан попасть в OpenUnacked этого же тика и
// сразу получить ступень 0, а не ждать следующего тика ради видимости
// освобождения (часы лесенки перезапускаются от dep_released_at — см.
// докблок SuppressedSource и GREATEST в host.OpenUnacked).
//
