...
func TestWriterAttributesDropsPerProject(t *testing.T) {
	...
	for _, p := range []int64{1, 0, 2} {
		w.Add(p, LogRecord{Timestamp: now, ObservedTS: now, Severity: "info", Body: "x"})
	}
	// Три новые записи project3 выбивают три самых старых: project1, project0
	// (не атрибутируется), project2.
	for i := 0; i < 3; i++ {
		w.Add(3, LogRecord{Timestamp: now, ObservedTS: now, Severity: "info", Body: "x"})
	}
