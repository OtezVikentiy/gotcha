...
// internal/ingest/timestamp.go (maxTimestampAge/maxTimestampFuture) — локальная копия,
// т.к. эти пакеты друг у друга не переиспользуются (см. capRunes в sanitize.go).
// Метрики (internal/metric/parse.go) с этим окном разошлись осознанно: там точка
// из будущего не отбрасывается, а приводится к моменту приёма, потому что читатели
// метрик (пороговый эвалюатор, графики) режут выборку по времени сервера и точка
// «из будущего» для них эквивалентна потерянной. Для логов такой правки не делалось:
// диапазон выборки задаёт пользователь.
//
