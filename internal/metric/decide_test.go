...
// Граница строгая — значение, равное порогу, не нарушение ни для gt, ни для lt (правило может задать
// порог, совпадающий с потолком метрики). Для открытого инцидента равенство — мёртвая зона, держим.
...
// Полоса гистерезиса берётся от модуля порога — умножение на (1±band) разворачивало бы её на
// отрицательных порогах, делая нарушение и восстановление одновременно истинными (см. decide.go).
...
// При нулевом пороге полосы нет, но нарушение и восстановление обязаны оставаться взаимоисключающими —
// «порог 0» значит «любое ненулевое значение — нарушение».
...
// Положительные пороги, самый частый случай, ведут себя ровно как прежде: полоса 5% ниже порога для gt.
